...
cpi <- read.csv(paste0(root,'cpi_deflators_by_year.csv'))
# Create variable for the 2013 CPI index
cpi.13 <- cpi |> filter(year == 2013) |> pull(index)
# Create variable for the Vintage year (the last year in the 5-year ACS) CPI Index
cpi.current <- cpi |> filter(year == vintage) |> pull(index)
# Calculate the CPI delfator
cpi.deflator <-  cpi.13 / cpi.current
# Remove CPI data no longer in use
rm(cpi.13,cpi.current)
...
# 2.0 Re-create Grouping Variables in the PUMS Data
- .1 Create UrbanSim Household Income Groupings

```{r}
#===============================================================================
# .1 UrbanSim Housing Income Groupings
...
# 3.0 Output Tables For UrbanSim
...
#4.0 Output Tables for Statewide Housing Plan Deliverables - .1 Number of Children in a Household - .2 Size of Household - .3 Tenure - .4 Number of Workers in a Household
