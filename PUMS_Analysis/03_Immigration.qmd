...
household_renter_burden_recent_immig <- pums %>%
 ...
 mutate(
    total_weight_by_entry = sum(total_weight, na.rm = TRUE),  # calculate the total weight for each nativity group
...
household_member_edu_recent_immig <- pums %>%
 ...
 group_by(Year.Entry) %>%
 mutate(
    total_weight_by_entry = sum(total_weight_by_edu_entry, na.rm = TRUE),  # calculate nativity group totals
...
householder_age_group_recent_immig <- pums %>%
  ...
  mutate(
    ...
    mutate (householder_age_range = factor(householder_age_range, levels = c(   # order groups in correct order
      ...
      "65-74",
      "75 and over"))) %>%
  arrange(householder_age_range, householder_entry)  # sort by age range and nativity
```
...
#Export age tables for recent immigrants
 write.xlsx(list('Recent immigrant hhld age' = householder_age_recent_immig,
           ...
           file = "J:/EPPR/Active Studies/EOHLC Statewide Hsg Plan_FY24/Data/pums_UMDI/Output/immig__recent_age_of_hhlder.xlsx") 
```
...
household_recent_immig_summary_table_RPA <- pums_RPA %>%
  ...
  summarise(total_by_entry = sum(first_weight, na.rm = TRUE), # sum the weights by nativity and year of entry
            .groups = "drop") %>%
group_by(rpa_acr) %>%
...
#Export rpa tables for recent immigrants
 write.xlsx(list('Recent immigrants by RPA' = household_recent_immig_summary_table_RPA), 
           file = "J:/EPPR/Active Studies/EOHLC Statewide Hsg Plan_FY24/Data/pums_UMDI/Output/immig__recent_RPA.xlsx") 
```
