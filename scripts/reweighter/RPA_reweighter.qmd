...
for (yr in c(2020,2030,2035,2040,2045,2050)){
  ...
  for (mid in mpos){
    outpath <- paste0(workpath, scen, '/Output_Files/', mid, '/')
    dt <- fread(paste0(outpath, '/reweighted_PUMS_', scen, '_', reb, '_', mid, '_', hdshp, '_', yr, '.csv'))
...
dhc <- d1 %>%
  ...
  mutate(
    total_available_units = total_households + Vacant_For_rent + Vacant_Rented_not_occupied + Vacant_For_sale_only + Vacant_Sold_not_occupied,
...
vax <- dsum101[,.(rpa_acr,total_units, total_households, total_owner_units, vacant_available_to_own, total_rental_units, vacant_available_to_rent, own_vac_rate_total, rent_vac_rate_total)]
...
uw[, own_units_needed_2035 := round(households_own_2035 * (1 + own_vac_rate_total))]
uw[, own_units_needed_2050 := round(households_own_2050 * (1 + own_vac_rate_total))]
uw[, rent_units_needed_2035 := round(households_rent_2035 * (1 + rent_vac_rate_total))]
uw[, rent_units_needed_2050 := round(households_rent_2050 * (1 + rent_vac_rate_total))]

uw[,new_owner_units_2035 := max(0, own_units_needed_2035 - total_owner_units),mpo]
uw[,new_rental_units_2035 := max(0, rent_units_needed_2035 - total_rental_units),mpo]
uw[,new_owner_units_2050 := max(0, own_units_needed_2050 - total_owner_units),mpo]
uw[,new_rental_units_2050 := max(0, rent_units_needed_2050 - total_rental_units),mpo]
uw[,total_new_units_needed_by_2035:=new_owner_units_2035+new_rental_units_2035]
uw[,total_new_units_needed_by_2050:=new_owner_units_2050+new_rental_units_2050]
uw[,scenario:=scen]

outfile <- uw[,.(scenario,mpo,total_units,total_owner_units,total_rental_units,vacant_available_to_own,vacant_available_to_rent,own_vac_rate_total,rent_vac_rate_total,households_own_2020,households_rent_2020,households_own_2030,households_rent_2030,households_own_2035,households_rent_2035,households_own_2040,households_rent_2040,households_own_2045,households_rent_2045,households_own_2050,households_rent_2050,new_owner_units_2035,new_rental_units_2035,new_owner_units_2050,new_rental_units_2050,total_new_units_needed_by_2035,total_new_units_needed_by_2050)]

setnames(outfile,c('total_units','total_owner_units','total_rental_units','vacant_available_to_own','vacant_available_to_rent','own_vac_rate_total','rent_vac_rate_total'),c('total_units_2020','total_owner_units_2020','total_rental_units_2020','vacant_available_to_own_2020','vacant_available_to_rent_2020','owner_unit_vacancy_rate_2020','rental_unit_vacancy_rate_2020'))
