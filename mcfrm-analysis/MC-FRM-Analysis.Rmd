...
#filter parcels to the MC-FRM 2030 Projected Stormwater Espspsps 
...
view(output_table[[98]])

obt<-rbindlist(output_table)%>%
  filter(!is.na(CITY))


ggplot(all_parcels_int)+geom_sf()+
  geom_sf(data = units_in_mcfrm70,
          fill = "darkblue")+
  geom_sf(data = units_in_mcfrm50,
          fill = "blue")+
  geom_sf(data= units_in_mcfrm30,
          fill = "lightblue")
