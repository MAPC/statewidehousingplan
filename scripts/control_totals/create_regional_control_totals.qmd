...
xw <- read_csv(paste0(output_path, 'PUMA10_RPA_crosswalk.csv')) |> 
  mutate(
...
write.csv(
  ...
  # Reads in the 2017-2021 PUMS Split Household data.
  pums_data <- read_csv(paste0(rwt_path, '/PUMS_data/PUMS', vintage, '.Split_formatted.csv'))
  ...
  hhtype_baseline <- pums_data |>
    # Filter for households and the head of household for each SERIALNO
    filter(TYPEHUGQ == "1" & SPORDER.adj == "1") |>
    mutate(RPA = rpa_acr) |>
    group_by(RPA, AgeCat) |>
    # Sum the household weights in the PUMS data.
    dplyr::count(
  ...
  tenure_baseline <- pums_data |>
    # Filter for households and the head of household for each SERIALNO
    filter(TYPEHUGQ == "1" & SPORDER.adj == "1" & ageCAT6 > 3) |>
    mutate(RPA = rpa_acr) |>
    group_by(RPA, AgeCat) |>
    # Sum the household weights in the PUMS data.
    dplyr::count(
  ...
  # Write baseline files
  # Output the baseline file for Household Population aggregations
  write.csv(hhpop_baseline, paste0(output_path, "hhpop_baseline.", hdshp, ".csv"), row.names = FALSE)
  
  # Output the baseline file for Household Type aggregations
  write.csv(hhtype_baseline, paste0(output_path, "hhtype_baseline.", hdshp, ".csv"), row.names = FALSE)
  
  # Output the baseline file for Age of Householder by tenure aggregations
  write.csv(tenure_baseline ,paste0(output_path, "tenure_baseline.", hdshp, ".csv"), row.names = FALSE)
  
  # Output the baseline file for Laborforce aggregations
  write.csv(lf_baseline, paste0(output_path, "lf_baseline.", hdshp, ".csv"), row.names = FALSE)
  #===============================================================================
  # 7.1.5-7 Baseline targeting files for 2020
  # 7.1.5 For 2020 - baseline file of households by age of householder by tenure
  age_baseline <- pums_data |>
  ...
  type_baseline <- pums_data |>
    ...
    mutate(
      RPA = as.factor(rpa_acr),
      HHtype.adj = as.factor(HHtype.adj)
    ) |>
    group_by(RPA) |>
    # Sum the household weights in the PUMS data.
    dplyr::count(
  ...
  #===============================================================================
  # Output the baseline file for 2020 Age of Householder by tenure aggregations
  write.csv(age_baseline,
            paste0(output_path, "age_baseline.", hdshp, ".csv"),
            row.names = FALSE)
  
  # Output the baseline file for 2020 Household size aggregations
  write.csv(size_baseline,
            paste0(output_path, "size_baseline.", hdshp, ".csv"),
            row.names = FALSE)
  
  # Output the baseline file for 2020 Household type only aggregations
  write.csv(type_baseline,
            paste0(output_path, "type_baseline.", hdshp, ".csv"),
            row.names = FALSE)
...
if (scen == "Scenario_1"){ # Scenario 1
  ...
  # hdsp.r_read(scenario = scen, file = "/headship_rates.1721.csv")


} else if (scen == "Scenario_2"){ # Scenario 2
  
  print("Using Headship Rates for Scenario 2.")
  ...
  hdsp.r <- hdsp.r_read(scenario = scen, file = "headship.rates.comparison_2000.hsr_to_1721.household.split.hsr.csv") |> 
    select(rpa_acr, ageCAT6, var, year, freq.scenario2) |> 
    dplyr::rename(freq = freq.scenario2) |> 
    mutate(
      ageCAT6 = as.factor(ageCAT6)
    )
  
  hdsp.r <- bind_rows(headship_rates, headship_rates.25, hdsp.r)
  
} else if (scen == "Scenario_3"){ # Scenario 3
  
  print("Using Headship Rates for Scenario 3.")
  
  hdsp.r_read(scenario = scen, file = "")
  
} else {  #Scenario 4
  
  print("Using Headship Rates for Scenario 4.")
...
proj.hh.MAPC97_by.mpo.tenure <- left_join(
   tenure_rates,
   hhds.MAPC97,
   by = c("rpa_acr" = "mpo", "AgeCat"), 
   relationship = "many-to-many"
  ) |> 
  rowwise() |>
  # Calculating the number of households in each RPA and age category
...
if (sum(tmp.1$hh) != sum(tmp.2$hh)-1) {
...
for (i in yrs) {
  if (i != 2020) {
    ...
    write.csv(
      target.file2,
      paste0(output_path,"tenure.targetfile.MAPC101.",scen,'.hdshp.',hdshp,'.y',i,".csv"),
      row.names = FALSE
    )
    ...
    # Write the reweighter targeting file to .csv by year i.
    fwrite(target.age, paste0(output_path,"age.targetfile.MAPC101.", scen,'.hdshp.',hdshp,'.y',i,".csv"))
    fwrite(target.type, paste0(output_path, "type.targetfile.MAPC101.", scen,'.hdshp.',hdshp,'.y',i,".csv"))
    fwrite(target.size,paste0(output_path,"size.targetfile.MAPC101.",scen,'.hdshp.',hdshp,'.y',i,".csv"))
...
for (i in yrs) {
  if(i!=2020){
    ...
    write.csv(
      target.file2,
      paste0(output_path, "tenure.targetfile.MAPC97.",scen,'.hdshp.',hdshp,'.y',i,".csv"),
      row.names = FALSE
    )
  ...
  if (i==2020){
    ...
    # Write the reweighter targeting file to .csv by year i.
    fwrite(target.age,paste0(output_path, "age.targetfile.MAPC97.",scen,'.hdshp.',hdshp,'.y',i,".csv"))
    fwrite(target.type,paste0(output_path, "type.targetfile.MAPC97.",scen,'.hdshp.',hdshp,'.y',i,".csv"))
    fwrite(target.size,paste0(output_path, "size.targetfile.MAPC97.",scen,'.hdshp.',hdshp,'.y',i,".csv"))
