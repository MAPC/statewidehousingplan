...
if(scen %in% c('Scenario_2', 'Scenario_3', 'Scenario_4')) {
  for (yr in seq(2030, 2050, 5)) {
    ...
    for (m in mpos) {
      dir.create(paste0(workpath, scen, '/Input_Files/', m),
                 showWarnings = F)
      dir.create(paste0(workpath, scen, '/Output_Files/', m),
                 showWarnings = F)
    }
...
for (yr in 2025) {
  for (mid in mpos) {
    ...
    pums <- fread(paste0(inpath, 'PUMS', vintage, '_', mid, '.csv'))
    if(scen=='Scenario_2'){pums[,SPORDER:=SPORDER.adj][,SERIALNO:=SERIALNO.adj]}
...
for (yr in c(2030, 2035, 2040, 2045, 2050)) {
  for (mid in mpos) {
    if(scen %in% c('Scenario_2','Scenario_3','Scenario_4')){
      infile1 <- paste0(workpath, 'Templates/reweighting_config_2021.json')
      outfile1 <- paste0(workpath, scen, '/Input_Files/', mid, '/reweighting_config_2021_', mid, '.json')
      cfig1 = readLines(infile1)
      cfig1 = gsub('PUMS2021.csv', paste0('PUMS2021_',scen,'_',mid,'_',yr,'.csv'), cfig1)
      writeLines(cfig1, outfile1)
    }
    # Set config
    inpath <- paste0(workpath, scen, '/Input_Files/', mid, '/')
...
for (yr in c(2020,2025)){
  for (mid in mpos) {
    ...
    # Use Scenario 1 outputs for 2020 and 2025
...
uw[,new_owner_units_needed_2020_to_2035 := own_units_needed_in_2035 - total_available_owner_units,mpo]
uw[,new_rental_units_needed_2020_to_2035 := rent_units_needed_in_2035 - total_available_rental_units,mpo]
uw[,new_owner_units_needed_2020_to_2050 := own_units_needed_in_2050 - total_available_owner_units,mpo]
uw[,new_rental_units_needed_2020_to_2050 := rent_units_needed_in_2050 - total_available_rental_units,mpo]
...
for (yr in c(2020,2025,2030,2035,2040,2045,2050)){
  tmp <- fread(paste0(workpath,scen,'/Output_Files/reweighted_PUMS_', scen, '_', reb, '_',hdshp,'_', yr, '.csv')) |> 
    select(year,reb,hdshp,scen,mpo,SERIALNO,SPORDER,tenure,TEN,new_WGTP)
  dt <- rbind(dt,tmp)
}
  setorder(dt,mpo,year)
  hh <- dt |> 
    filter(SPORDER==1) |> 
    mutate(tenure = case_when(
      TEN %in% 1:2 ~ 'own',
...
of[mpo=='Statewide',`owner unit vacancy rate 2020`:=`vacant available to own` / `total available owner units 2020`]
of[mpo=='Statewide',`rental unit vacancy rate 2020`:=`vacant available to rent` / `total available rental units 2020`]
of[mpo=='Statewide',`combined vacancy rate 2020`:=(`vacant available to own` + `vacant available to rent`) / (`total available owner units 2020` + `total available rental units 2020`)]
...
fwrite(of, paste0(workpath, scen, '/Output_Files/',scen,'_unit_demand_estimates.csv'))
}
