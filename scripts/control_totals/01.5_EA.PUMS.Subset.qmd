...
for (i in c("BRPC")) {
  ...
  # - Total population of the PUMS sample comes within 10% of the total population target.
  # - 5000 iterations of the repeat loop are reached.
  repeat {
    ...
    for (j in agegroup.list) {
      ...
      # Samples households with replacement equal to the number of households in the target (for RPA and Age Group). Adds the iteration number to the dataset.
      sample.pums <- slice_sample(tmp.pums, n = hh.target, replace = TRUE) |> mutate(iter = iteration)
      ...
      rm(tmp.pums, sample.pums, tmp.pop, tmp.hh, tmp.child_0004, tmp.child_0509, tmp.child_1014, tmp.child_1517)
   ...
   if ((pop.target - pop.total) < round(pop.target*0.1, 0) | iteration > 5000) {break}
