...
#GEOIDs for 2010 Decennial Census
...
#GEOIDs for 2020 Decennial Census
...
decennial_call <- function(decennial.var_name, year){
  if (year == 2010){
    get_decennial(
...
dec.vars <- function(vars, con){
  vars |>  
  filter(grepl(pattern = paste0("^",con,"$"), concept)) |> 
  select(name) |> 
  mutate(
...
umdi_pop<- fread(paste0(root, "UMDI/UMDI_V2024_MCD_Age_Sex_detail_2010-2050_a.csv")) |>
  #Select years of interest - decadal intervals
  #ageCAT6 refers to 5 year age groups (0-4, 5-9, 10-14, etc.)
  select(muni_name,muni_id,ageCAT6,Sex,`2010`,`2020`,`2030`,`2040`,`2050`) |> 
...
#TODO: 
#1. Population checks using Census data for QA/QC?
```
...
pop.values <- decennial_call(pop.vars, 2010)
...
#Queries the Census API via {tidycensus} to retrieve data about the population
pop.values <- decennial_call(pop.vars, 2020)
...
#Queries the Census API via {tidycensus} to retrieve data about the population
#in households
pa.values <- decennial_call(pa.vars, 2010)
...
#Queries the Census API via {tidycensus} to retrieve data about the population
#in households
pa.values <- decennial_call(pa.vars, 2020)
...
# Group Quarters Population - Correctional Facilities
```{r}
#2010
PCO003.vars <- dec.vars(vars.dec10, "GROUP QUARTERS POPULATION IN CORRECTIONAL FACILITIES FOR ADULTS BY SEX BY AGE")

PCO003.labels <- dec.labels("2010", "GROUP QUARTERS POPULATION IN CORRECTIONAL FACILITIES FOR ADULTS BY SEX BY AGE") |> gq.clean() 

PCO003.values <- decennial_call(PCO003.vars, 2010)
...
#Group Quarters Population - Juvenile Facilities
```{r}
#2010
"PCO004"
...
# Group Quarters Population - Nursing Facilities
```{r}
#2010
"PCO005"
...
#Group Quarters Population - Other Institutionalized Facilities
```{r}
#2010
"PCO006"
...
# Group Quarters Population - Military
```{r}
#2010
"PCO009"
...
#Group Quarters Population - Other Non-Institutionalized 
```{r}
#2010
"PCO010"
...
# Calculate the Percentage of People in Households
```{r}
hr.10
#TODO:
#1. Join 2010 and 2020 Household Population data to the UMDI MCD
#level population projections.
#2. Calculate % of people in households in each age/sex group for each
#decadal period. 
```
...
pums_data <- get_pums(
  ...
  mutate(
    ...
    ageCAT6 =
      cut(AGEP, breaks = c(-Inf, seq(4, 84, 5), Inf), labels = 1:18),
    ...
    PAGEC2 =
      cut(AGEP, breaks = c(-Inf, 14, 19, 24, 34, 44, 54, 64, 74, 79, 84, Inf),
...
#Baseline of Households by Household Type
