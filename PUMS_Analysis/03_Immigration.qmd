...
1) Identify households whose head of household is foreign born
...
household_foreign_born_summary_table <- pums %>%
  filter(RT != "H") %>%       # exclude hhld data points because nativity is a person-level data point
  group_by(SERIALNO) %>%      # group by household 
  summarise(first_weight = first(WGTP, na.rm = TRUE),     # get the first person's weight for hhld (head of household)
...
households_foreign_born <- pums %>%
 filter(RT != "H") %>%   # exclude hhld data points
 group_by(SERIALNO) %>% 
 summarise(
...
2) Characteristics and demographics of households that have a foreign-born head of household
...
household_type_foreign_born <- pums %>%
  ...
  group_by(SERIALNO) %>%  # group by hhld
  summarise(
    household_type = first(HHT, na.rm = TRUE),             # get the first person's household type (householder)
    ...
    .groups = "drop") %>%
  group_by(householder_nativity, household_type) %>%       # group by nativity and hhld type
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),        # sum total weights for each nativity and type
    .groups = "drop") %>%
...
#Export hhld type table
 write.xlsx(list('Hhld type' = household_type_foreign_born), 
...
householder_age_foreign_born <- pums %>%
 ...
 summarise(
  ...
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),        # sum total weights for each nativity and age range
    .groups = "drop") %>%
  group_by(householder_nativity) %>%                 # group by nativity to calculate totals within each group
  mutate(
    total_weight_by_nativity = sum(total_weight, na.rm = TRUE),  # calculate the total weight for each nativity group
...
householder_race_foreign_born <- pums %>%
 ...
 summarise(
  ...
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),        # sum total weights for each nativity and race
    .groups = "drop") %>%
  group_by(householder_nativity) %>%       # group by nativity to calculate race totals within each group
  mutate(
    total_weight_by_nativity = sum(total_weight, na.rm = TRUE),  # calculate the total weight for each nativity group
    percentage_within_nativity = (total_weight / total_weight_by_nativity) * 100  # calculate percentage within each group
  ) %>%
  ...
  mutate(householder_race = case_when(  # rename race categories
    ...
    householder_race == 3 ~ "American Indian alone",
...
householder_hisp_foreign_born <- pums %>%
  ...
  mutate(householder_hisp = case_when(     # rename race categories, and group detailed Hispanic into one category
    ...
    as.numeric(householder_hisp) >= 2 & as.numeric(householder_hisp) <= 24 ~ "Hispanic or Latino", # convert to numeric to group
...
householder_race_non_hisp <- pums %>%
 filter(RT != "H") %>%   # exclude hhlds 
 filter(as.numeric(HHLDRHISP) == 1) %>%  # for non-hispanic hhldrs only
 select(uniq_obs, SERIALNO, PUMA, WGTP, HHLDRRAC1P, HHLDRHISP, NATIVITY) %>% 
 group_by(SERIALNO) %>% 
 summarise(
    householder_race = first(HHLDRRAC1P, na.rm = TRUE),    # get the first person's householder race
...
#Export race and ethnicity tables 
 write.xlsx(list('Race of hhldr by nativity' = householder_race_foreign_born, 
                 'Ethnicity of hhldr by nativity' = householder_hisp_foreign_born, 
                 'Race of non-hisp hhldr' = householder_race_non_hisp), 
...
household_year_entry_foreign_born <- pums %>%
 ...
 mutate(Year.Entry = case_when(      # create year of entry groups
      ...
      YOEP >= 1934 & YOEP <= 1953 ~ "1934 to 1953",
      YOEP >= 1954 & YOEP <= 1973 ~ "1954 to 1973",
      YOEP >= 1974 & YOEP <= 1993 ~ "1974 to 1993",
      YOEP >= 1994 & YOEP <= 2003 ~ "1994 to 2003",
      YOEP >= 2004 & YOEP <= 2013 ~ "2004 to 2013",
      YOEP >= 2014 & YOEP <= 2024 ~ "2014 and later",
      TRUE ~ "0")) %>%
 select(uniq_obs, SERIALNO, WGTP, NATIVITY, Year.Entry) %>% 
 group_by(SERIALNO) %>% 
 summarise(
    householder_entry = first(Year.Entry, na.rm = TRUE),   # get the first person's year of entry group (householder)
    householder_nativity = first(NATIVITY, na.rm = TRUE),  # get nativity of first person (householder)
    first_weight = first(WGTP, na.rm = TRUE),              # get the weight for the first person (householder)
    .groups = "drop") %>%
  ...
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),        # sum total weights for each nativity and year of entry group
    .groups = "drop") %>%
  group_by(householder_nativity) %>%                       # group by nativity to calculate totals within each group
  mutate(
    total_weight_by_nativity = sum(total_weight, na.rm = TRUE),  # calculate the total weight for each nativity group
...
#Export year of entry table 
 write.xlsx(list('Year entry of hhldr' = household_year_entry_foreign_born), 
           file = "J:/EPPR/Active Studies/EOHLC Statewide Hsg Plan_FY24/Data/pums_UMDI/Output/immig_year_entry_hhldr.xlsx") 
```


```{r}
#Size of hhld - NP is number of persons associated with housing record
...
household_size_foreign_born <- pums %>%
 ...
 summarise(
  ...
  mutate(household_size_category = case_when(              # create household size categories
    ...
    household_size < 10 ~ as.character(household_size),    # sizes less than 10 as strings to categorize them
    TRUE ~ "Unknown")) %>%
  group_by(householder_nativity, household_size_category) %>%  # group by nativity and size
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),            # sum total weights for each nativity and size
    .groups = "drop") %>%
  group_by(householder_nativity) %>%                       # group by nativity to calculate totals within each group
  mutate(
    total_weight_by_nativity = sum(total_weight, na.rm = TRUE),  # calculate the total weight for each nativity group
...
median_household_size_by_nativity <- pums %>%
  ...
  summarise(
    median_household_size = median(household_size, na.rm = TRUE),  # calculate median household size
    .groups = "drop"
  ) %>%
  mutate(householder_nativity = case_when(  # rename nativity categories
    householder_nativity == 1 ~ "Native",
...
household_children_foreign_born <- pums %>%
 ...
 summarise(
    ...
    first_weight = first(WGTP, na.rm = TRUE),              # get the weight for the first person (householder)
    .groups = "drop") %>%
  group_by(householder_nativity, household_children) %>%   # group by nativity and children category
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),        # sum total weights for each nativity and children category
    .groups = "drop") %>%
  group_by(householder_nativity) %>%                       # group by nativity to calculate totals within each group
  mutate(
    total_weight_by_nativity = sum(total_weight, na.rm = TRUE),  # calculate the total weight for each nativity group
...
householder_income_foreign_born <- pums %>%
 ...
 group_by(SERIALNO) %>% 
 summarise(
    householder_income = first(HH.Income.Grp, na.rm = TRUE),  # get the first person's hhld income
    householder_nativity = first(NATIVITY, na.rm = TRUE),     # get nativity of first person (householder)
    first_weight = first(WGTP, na.rm = TRUE),                 # get the weight for the first person (householder)
    .groups = "drop") %>%
  group_by(householder_nativity, householder_income) %>%      # group by nativity and income
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),           # sum total weights for each nativity and income
    .groups = "drop") %>%
  group_by(householder_nativity) %>%                 # group by nativity to calculate totals within each group
  mutate(
    total_weight_by_nativity = sum(total_weight, na.rm = TRUE),  # calculate the total weight for each nativity group
...
householder_tenure_foreign_born <- pums %>%
 ...
 group_by(SERIALNO) %>% 
 summarise(
    householder_tenure = first(TEN, na.rm = TRUE),         # get the first person's tenure for hhld
    householder_nativity = first(NATIVITY, na.rm = TRUE),  # get nativity of first person (householder)
    first_weight = first(WGTP, na.rm = TRUE),              # get the weight for the first person (householder)
    .groups = "drop") %>%
  group_by(householder_nativity, householder_tenure) %>%   # group by nativity and tenure
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),        # sum total weights for each nativity and tenure
    .groups = "drop") %>%
  group_by(householder_nativity) %>%              # group by nativity to calculate totals within each group
  mutate(
    total_weight_by_nativity = sum(total_weight, na.rm = TRUE),       # calculate the total weight for each nativity group
...
#Export tenure table
 write.xlsx(list('Tenure of householder' = householder_tenure_foreign_born), 
           file = "J:/EPPR/Active Studies/EOHLC Statewide Hsg Plan_FY24/Data/pums_UMDI/Output/immig_tenure_hhldr.xlsx") 
```
...
household_renter_burden_foreign_born <- pums %>%
 ...
 group_by(SERIALNO) %>% 
 summarise(
    householder_renter_burden = first(Cost.Burden, na.rm = TRUE),  # get the first person's cost burden (householder)
    householder_nativity = first(NATIVITY, na.rm = TRUE),          # get nativity of first person (householder)
    first_weight = first(WGTP, na.rm = TRUE),                      # get the weight for the first person (householder)
    .groups = "drop") %>%
  group_by(householder_nativity, householder_renter_burden) %>%    # group by nativity and cost burden group
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),                # sum total weights for each nativity and cost burden
    .groups = "drop") %>%
  group_by(householder_nativity) %>%                           # group by nativity to calculate totals within each group
  mutate(
    total_weight_by_nativity = sum(total_weight, na.rm = TRUE),    # calculate the total weight for each nativity group
...
household_owner_burden_foreign_born <- pums %>%
 ...
 group_by(SERIALNO) %>% 
 summarise(
    householder_owner_burden = first(Cost.Burden, na.rm = TRUE),  # get the first person's cost burden (householder)
    householder_nativity = first(NATIVITY, na.rm = TRUE),         # get nativity of first person (householder)
    first_weight = first(WGTP, na.rm = TRUE),                     # get the weight for the first person (householder)
    .groups = "drop") %>%
...
```{r}
#English language proficiency - ENG
household_english_foreign_born <- pums %>%
 filter(RT != "H") %>%   # exclude hhlds
 select(uniq_obs, SERIALNO, WGTP, NATIVITY, ENG) %>% 
 group_by(SERIALNO) %>% 
 summarise(
    householder_english = first(ENG, na.rm = TRUE),        # get the first person's english level (householder)
    householder_nativity = first(NATIVITY, na.rm = TRUE),  # get nativity of first person (householder)
    first_weight = first(WGTP, na.rm = TRUE),              # get the weight for the first person (householder)
    .groups = "drop") %>%
  filter(householder_nativity == 2) %>%                    # only including foreign born hhlds 
  group_by(householder_nativity, householder_english) %>%  # group by nativity and english level
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),        # sum total weights for each nativity and english level
    .groups = "drop") %>%
  group_by(householder_nativity) %>%                       # group by nativity to calculate totals within each group
  mutate(
    total_weight_by_nativity = sum(total_weight, na.rm = TRUE),  # calculate the total weight for each nativity group
...
```{r}
#English spoken at home - LANX is language other than english spoken at home
household_language_foreign_born <- pums %>%
 filter(RT != "H") %>%   # exclude hhlds
 select(uniq_obs, SERIALNO, WGTP, NATIVITY, LANX) %>% 
 group_by(SERIALNO) %>% 
 summarise(
    householder_language = first(LANX, na.rm = TRUE),      # get the first person's language (householder)
    householder_nativity = first(NATIVITY, na.rm = TRUE),  # get nativity of first person (householder)
    first_weight = first(WGTP, na.rm = TRUE),              # get the weight for the first person (householder)
    .groups = "drop") %>%
  #filter(householder_nativity == 2) %>%       # if only including foreign born hhlds 
  group_by(householder_nativity, householder_language) %>% # group by nativity and language
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),        # sum total weights for each nativity and language
    .groups = "drop") %>%
  group_by(householder_nativity) %>%                 # group by nativity to calculate totals within each group
  mutate(
    total_weight_by_nativity = sum(total_weight, na.rm = TRUE),  # calculate the total weight for each nativity group
...
```{r}
#Limited english speaking hhld - LNGI is a hhld variable for limited english speaking household
household_limited_lang_foreign_born <- pums %>%
 filter(RT != "H") %>%   # exclude hhlds
 select(uniq_obs, SERIALNO, WGTP, NATIVITY, LNGI) %>% 
 group_by(SERIALNO) %>% 
 summarise(
    householder_language = first(LNGI, na.rm = TRUE),      # get the first person's language (householder)
    householder_nativity = first(NATIVITY, na.rm = TRUE),  # get nativity of first person (householder)
    first_weight = first(WGTP, na.rm = TRUE),              # get the weight for the first person (householder)
    .groups = "drop") %>%
  filter(householder_nativity == 2) %>%                     # only including foreign born hhlds 
...
household_world_origin_foreign_born <- pums %>%
 ...
 group_by(SERIALNO) %>% 
 summarise(
    householder_origin = first(WAOB, na.rm = TRUE),        # get the first person's place of origin (householder)
    householder_nativity = first(NATIVITY, na.rm = TRUE),  # get nativity of first person (householder)
    first_weight = first(WGTP, na.rm = TRUE),              # get the weight for the first person (householder)
    .groups = "drop") %>%
  group_by(householder_nativity, householder_origin) %>%   # group by nativity and place of origin
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),  # summarize total weights for each nativity and place of origin
    .groups = "drop") %>%
  group_by(householder_nativity) %>%                 # group by nativity to calculate totals within each group
  mutate(
    total_weight_by_nativity = sum(total_weight, na.rm = TRUE),       # calculate the total weight for each nativity group
...
householder_edu_foreign_born <- pums %>%
 ...
 group_by(SERIALNO) %>% 
 summarise(
    householder_edu = first(EDU, na.rm = TRUE),            # get the first person's educational attainment (householder)
    householder_nativity = first(NATIVITY, na.rm = TRUE),  # get nativity of first person (householder)
    first_weight = first(WGTP, na.rm = TRUE),              # get the weight for the first person (householder)
    .groups = "drop") %>%
  group_by(householder_nativity, householder_edu) %>%      # group by nativity and edu attainment
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE)) %>%  # summarize total weights for each nativity and edu attainment
  group_by(householder_nativity) %>%                     # group by nativity to calculate totals within each group
  mutate(
    total_weight_by_nativity = sum(total_weight, na.rm = TRUE),  # calculate the total weight for each nativity group
    percentage_within_nativity = (total_weight / total_weight_by_nativity) * 100) %>%  # calculate percentage within each group
...
```{r}
#Edu attainment for all household members, not just head of hhld
household_member_edu_foreign_born <- pums %>%
 ...
 summarise(
    total_weight = sum(WGTP, na.rm = TRUE),          # sum weights for each edu level within households - not just 1st person
    .groups = "drop") %>%
...
3) Identify households whose head of household is foreign born in each RPA

```{r}
#Calculate the total number of households in sample by RPA
household_data_RPA <- pums_RPA %>%
  filter(RT == "H") %>%
  filter(TYPEHUGQ == 1) %>%  # no group quarters
  filter(NP > 0) %>%         # excluding vacant units
  group_by(rpa_acr) %>%
  summarise(total_weighted_sum = sum(WGTP)) 

#calculate number of foreign born heads of hhlds for each RPA
...
#combine tables and calculate share of foreign hhldrs of all hhlds
...
# Sub group analysis of recent immigrants

4) Analysis of hhlds where the head of hhld is a recent immigrant: year of entry 2014 or more recent

```{r}
#Cost burden for recent immigrants - head of hhld - renter and owner cost burden - GRPIP OCPIP
...
household_renter_burden_recent_immig <- pums %>%
 ...
 summarise(
    total_weight = sum(first_weight, na.rm = TRUE),       # sum total weights for each nativity, year entry, and cost burden
    .groups = "drop") %>%
 group_by(householder_nativity, year_entry) %>%     # group by nativity and year entry to calculate totals within each group
 mutate(
    total_weight_by_entry = sum(total_weight, na.rm = TRUE),                  # calculate the total weight for each group
...
householder_tenure_recent_immig <- pums %>%
 ...
 group_by(SERIALNO) %>% 
 summarise(
    householder_tenure = first(TEN, na.rm = TRUE),         # get the first person's tenure (householder)
    householder_nativity = first(NATIVITY, na.rm = TRUE),  # get nativity of first person (householder)
    first_weight = first(WGTP, na.rm = TRUE),              # get the weight for the first person (householder)
    year_entry = first(Year.Entry, na.rm = TRUE),          # get the year entry for the first person (householder)
    .groups = "drop") %>%
 ...
 summarise(
    total_weight = sum(first_weight, na.rm = TRUE),        # sum total weights by year entry and tenure
    .groups = "drop") %>%
 group_by(year_entry) %>%                                  # group by year entry for weight totals
 mutate(
    total_weight_by_entry = sum(total_weight, na.rm = TRUE),       # calculate the total weight for each year entry group
    percentage_within_nativity = (total_weight / total_weight_by_entry) * 100) %>%  # calculate percentage within each group
...
household_size_recent_immig <- pums %>%
 ...
 summarise(
  ...
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),        # sum total weights for each size and year of entry group
    .groups = "drop") %>%
...
household_member_edu_recent_immig <- pums %>%
 ...
 summarise(
    total_weight_by_edu_entry = sum(total_weight, na.rm = TRUE),  # total weight by edu and nativity group
    .groups = "drop") %>%
 group_by(Year.Entry) %>%
 mutate(
    total_weight_by_entry = sum(total_weight_by_edu_entry, na.rm = TRUE),                    # calculate group totals
    percentage_within_entry = (total_weight_by_edu_entry/ total_weight_by_entry) * 100) %>%  # calculate percentage
    mutate (EDU = factor(EDU, levels = c(   # order groups in correct order
     "Less than high school diploma",
     "High school diploma or equivalent",
     "Some college",
...
householder_age_recent_immig <- pums %>%
 filter(RT != "H") %>%             # exclude hhlds
 mutate(Year.Entry = case_when(    # create year of entry groups
      YOEP < 1934 ~ "Before 1934",
      YOEP >= 1934 & YOEP <= 2013 ~ "1934 to 2013",
      YOEP >= 2014 & YOEP <= 2024 ~ "2014 and later",
      TRUE ~ "0")) %>%
 select(uniq_obs, SERIALNO, WGTP, HHLDRAGEP,NATIVITY, Year.Entry) %>% 
 mutate(age_range = case_when(                  # create age ranges
    HHLDRAGEP >= 0 & HHLDRAGEP < 5 ~ "Under 5",
    HHLDRAGEP >= 5 & HHLDRAGEP < 18 ~ "5-17",
    HHLDRAGEP >= 18 & HHLDRAGEP < 35 ~ "18-34",
 ...
 summarise(
    householder_age = first(HHLDRAGEP, na.rm = TRUE),      # get the first person's householder age
    householder_entry= first(Year.Entry, na.rm = TRUE),    # get year of entry of first person (householder)
    householder_nativity = first(NATIVITY, na.rm = TRUE),  # get nativity of first person (householder)
    first_weight = first(WGTP, na.rm = TRUE),              # get the weight for the first person (householder)
    age_range = first(age_range, na.rm = TRUE),            # get age range for the first person (householder)
    .groups = "drop") %>%
  filter(householder_nativity == 2) %>%                    # filter for foreign-born householders only
  group_by(householder_entry, age_range) %>%               # group by year of entry and age range
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),        # summarize total weights for each age range and year of entry
    .groups = "drop") %>%
  group_by(householder_entry) %>%                          # group to calculate totals within each group
  mutate(
    total_weight_by_entry = sum(total_weight, na.rm = TRUE),             # calculate the total weight for each group
    percentage_within_entry = (total_weight / total_weight_by_entry) * 100  # calculate percentage within each group
  ) %>%
  mutate (age_range = factor(age_range, levels = c(        # order groups in correct order
      ...
      "65-74",
      "75 and over"))) %>%
   arrange(householder_entry, age_range)
```

```{r}
#Age part 2
#Share of each foreign born age group that is recent immigrants
householder_age_group_recent_immig <- pums %>%
  filter(RT != "H") %>%            # exclude hhlds
  mutate(Year.Entry = case_when(   # create year of entry groups
      YOEP < 1934 ~ "Before 1934",
      YOEP >= 1934 & YOEP <= 2013 ~ "1934 to 2013",
      YOEP >= 2014 & YOEP <= 2024 ~ "2014 and later",
      TRUE ~ "0")) %>%
  select(RT, uniq_obs, SERIALNO, WGTP, HHLDRAGEP, NATIVITY, Year.Entry) %>% 
  mutate(age_range = case_when(
    HHLDRAGEP >= 0 & HHLDRAGEP < 5 ~ "Under 5",
    HHLDRAGEP >= 5 & HHLDRAGEP < 18 ~ "5-17",
    HHLDRAGEP >= 18 & HHLDRAGEP < 35 ~ "18-34",
  ...
  summarise(
    householder_age_range = first(age_range, na.rm = TRUE),  # get age range of the householder
    householder_nativity = first(NATIVITY, na.rm = TRUE),    # get nativity of the householder
    householder_entry = first(Year.Entry, na.rm = TRUE),     # get year of entry for the householder
    first_weight = first(WGTP, na.rm = TRUE),                # get weight for the householder
    .groups = "drop") %>%
  filter(householder_nativity == 2) %>%                      # filter for foreign-born householders only
  group_by(householder_age_range, householder_entry) %>%     # group by age range and year of entry
  summarise(
    total_weight = sum(first_weight, na.rm = TRUE),          # calculate total weight for each age and entry group
    .groups = "drop") %>%
  group_by(householder_age_range) %>%          # group by age range to calculate percentages within each age group
  mutate(
    total_weight_by_age_range = sum(total_weight, na.rm = TRUE),                         # total weight by age group
    ...
    mutate (householder_age_range = factor(householder_age_range, levels = c(   # order groups in correct order
      ...
      "65-74",
      "75 and over"))) %>%
  arrange(householder_age_range, householder_entry)  # sort by age range and entry
```

```{r}
#Export age tables for recent immigrants
 write.xlsx(list('Recent immigrant hhld age' = householder_age_recent_immig,
                 'Share of recent immig by age' = householder_age_group_recent_immig), 
           file = "J:/EPPR/Active Studies/EOHLC Statewide Hsg Plan_FY24/Data/pums_UMDI/Output/immig_recent_age_of_hhlder.xlsx") 
```
...
household_recent_immig_summary_table_RPA <- pums_RPA %>%
  filter(RT != "H") %>%            # exclude hhld data points
  group_by(SERIALNO, rpa_acr) %>%  # group by household and RPA
  summarise(first_weight = first(WGTP, na.rm = TRUE), # get the first person's weight for household
            NATIVITY = first(NATIVITY),               # get the nativity for the first person
            YOEP = first(YOEP, na.rm = TRUE),         # get the year of entry for the first person
            .groups = "drop") %>%           
  mutate(Year.Entry = case_when(                      # create year of entry groups
      YOEP < 1934 ~ "Before 1934",
      YOEP >= 1934 & YOEP <= 2013 ~ "1934 to 2013",
      YOEP >= 2014 & YOEP <= 2024 ~ "2014 and later",
      TRUE ~ "0")) %>%
  filter(NATIVITY == 2) %>%                                   # filter for foreign born only
  group_by(NATIVITY, rpa_acr, Year.Entry) %>%                 # group by rpa and year of entry
  summarise(total_by_entry = sum(first_weight, na.rm = TRUE), # sum the weights by nativity and year of entry and rpa
            .groups = "drop") %>%
group_by(rpa_acr) %>%
  mutate(total_foreign_born_weight = sum(total_by_entry)) %>%  # calculate total foreign-born weight per RPA
  ungroup() %>%
  mutate(percentage = (total_by_entry / total_foreign_born_weight) * 100)
  #select(-total_foreign_born_weight)  # drop total column 
```

```{r}
#Export rpa tables for recent immigrants
 write.xlsx(list('Recent immigrants by RPA' = household_recent_immig_summary_table_RPA), 
           file = "J:/EPPR/Active Studies/EOHLC Statewide Hsg Plan_FY24/Data/pums_UMDI/Output/immig_recent_RPA.xlsx") 
```
