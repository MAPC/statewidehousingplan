...
```{r}
# 2020 is going to look like the normal pums
# 2025 is going to be remove og and adj SERIALNO from normal pums, replace with og and adj for the split households BUT ONLY THE SAMPLE FOR THAT YEAR. Keep the others in their normals households.
# 2030 is going to remove og and adj serial from normal pums from 2025 and 2030 and replace with og and adj from split househods CUMULATIVE NOW. Keep the others in normal households.
...
for (i in years) {
  ...
  if(i == 2020){
    ...
    done_pums <- bind_rows(tmp_pums, og, pums.sample_2025) |> 
      mutate(
        SERIALNO.adj = coalesce(SERIALNO)
      )
