...
for (yr in c(2025,2035)){
    ...
    dx[,pid:=paste0(SERIALNO,'_',SPORDER)]
    
    dfrac <- dx[WGT2>0]
    dfrac[,new_WGTP:=WGT2]
    dfrac[,hid:=paste0(SERIALNO,'_','frac')]
    
    xr <- dx[rep(1:.N,WGT1)]
    xr[,new_WGTP:=1]
    
    xr[, row := .I]
    xr[, hid := paste(pid, row, sep = '_')]
    xr <- xr[,.(pid,hid)]
    setkey(xr,pid)
    setkey(dx,pid)
    dw <- dx[xr]
    # xr <- xr[,.(SERIALNO,hid)]
    # dw <- merge(xr,dx,by='pid', all=T, allow.cartesian = T)
    ...
    if(scen=='Scenario_1'){
    tar <- fread(paste0(ctrltotal.path,'hhtype.targetfile.MAPC101.',scen,'.hdshp.acs1721.y',yr,'.csv'))
    }
    if(scen!='Scenario_1'){
    tar <- fread(paste0(ctrltotal.path,'hhtype.targetfile.MAPC101.',scen,'.hdshp.scen2.y',yr,'.csv'))
    }
    
    hh <- dt[SPORDER==1, lapply(.SD,lun),.(mpo,ageCAT6,HHtype),.SDcols='hid']
    setkey(hh,mpo,ageCAT6,HHtype)
    setnames(hh,'hid','HH')
    
    tar[,ageCAT6:=as.numeric(gsub('x == ','',AgeCat))][,HHtype:=as.numeric(gsub('x == ','',HHtype))][,mpo:=RPA]
    setkey(tar,mpo,ageCAT6,HHtype)
...
# 5 - Post-processor 03 - person shifts
...
for (yr in c(2025,2035)){
  ...
  swm <- fread(paste0(output_path,'expanded_reweighted_PUMS_',yr,'.csv'))
  ...
  saveRDS(dt_shift,paste0(output_path,'/pop_shifted_',run,'_',yr,'_v3.rds'))
}
...
# 6 - Post-processor 04 - person duplication
