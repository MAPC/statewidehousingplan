...
clean_ethnicity <- keys_import %>%
  mutate(income = income*1000) %>% 
  mutate(ethnicity = case_when(derived_ethnicity == "Hispanic or Latino" ~ "lat",
                                derived_ethnicity == "Not Hispanic or Latino" ~ "nonlat",
                                derived_ethnicity == "Ethnicity Not Available" ~ "na",
                                derived_ethnicity == "White" ~ "na",
                                derived_ethnicity == "Joint" ~ "na",
                                derived_ethnicity == "Free Form Text Only" ~ "na")) 

clean_race <- clean_ethnicity %>%
  mutate(race = case_when(
                               derived_ethnicity == "White" & derived_race == "Joint" ~ "whi",
    
                               derived_race == "White" ~ "whi",
                               derived_race == "Black or African American" ~ "baa",
                               derived_race == "Asian" ~ "asn",
                               derived_race == "Native Hawaiian or Other Pacific Islander" ~ "nhp",
                               derived_race == "American Indian or Alaska Native" ~ "nav",
                               derived_race == "Joint" ~ "jnt",
                               derived_race == "Joint" ~ "jnt",
                               derived_race == "Race Not Available" ~ "na",
                               derived_race == "2 or more minority races" ~ "mlt",
                               derived_race == "Free Form Text Only" ~ "na"))
