...
pa.vars <- vars.dec10 |>  
  filter(concept=='SEX BY AGE FOR THE POPULATION IN HOUSEHOLDS') |> 
  select(name) |> 
  mutate(
...
#Creates a list of sex by age population variables to use in the API query
pa.vars <- vars.dec20 |>  
  filter(concept=='SEX BY AGE FOR SELECTED AGE CATEGORIES') |> 
  select(name) |> 
  mutate(
    name = as.character(name)
  ) |>  
...
pa.labels <- vars.dec20 |>  
  ...
  filter(
    !name %in% c("P12_001N", "P12_002N", "P12_026N")
  ) |> 
  select(
    -c(sex1, sex2)
  ) |>  
