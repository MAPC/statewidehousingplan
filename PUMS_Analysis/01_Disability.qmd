...
household_income_disability <- people_with_disability %>%
  ...
  summarise(total_by_income = sum(first_weight, na.rm = TRUE), # Sum the weights by income
    .groups = "drop") %>%   
  ...
  mutate(percentage_of_total = (total_by_income / total_households_with_disability) * 100)  # Calculate percent of disabled households
```
...
household_tenure_disability <- people_with_disability %>%
  ...
  mutate(TEN = case_when(                  #Rename tenure variables 
    ...
    TRUE ~ NA_character_)) %>%
  filter(!is.na(TEN)) %>%
  mutate(percentage_of_total = (total_by_tenure / total_households_with_disability) * 100)  # Calculate percent of disabled households
```
...
household_age_of_housing_disability <- people_with_disability %>%
  ...
  group_by(SERIALNO) %>% # group by household 
  summarise(first_weight = first(WGTP, na.rm = TRUE), # get the first person's weight for hhld
    Housing.Age = first(Housing.Age), .groups = "drop") %>%          # get the tenure for the first person                          
  group_by(Housing.Age) %>%                                  # group by TEN to calculate total weights for each tenure type
  summarise(total_by_house_age = sum(first_weight, na.rm = TRUE), # Sum the weights by tenure
    .groups = "drop") %>%  
  mutate(Housing.Age = factor(Housing.Age, levels = c(    #order groups in correct order
      ...
      "2000 and later"))) %>%
  arrange(Housing.Age) %>% 
  mutate(percentage_of_total = (total_by_house_age / total_households_with_disability) * 100)  # Calculate percent of disabled households
```

```{r}
#Cost burden for households with disability - GRPIP and OCPIP 
