...
```{r}
#Joining the PUMA to RPA crosswalk to the PUMS data. - not working right now
#converting puma to numeric 
xw$PUMA <- as.numeric(xw$PUMA)

#join RPA croswalk to pums data
pums_RPA <- left_join(
  pums,
  xw,
  by = c("PUMA"))
...
household_tenure_disability <- people_with_disability %>%
  ...
  summarise(total_weight_by_tenure = sum(first_weight, na.rm = TRUE), # Sum the weights by tenure
    .groups = "drop")                                       

household_tenure_disability_table <- household_tenure_disability %>%
...
#may want to add a percent column for tenure 
