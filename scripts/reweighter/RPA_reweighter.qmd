...
Let RVRY = Regional Target Vacancy Rate for region R in year Y Avail_units_2020 = hu_occ 2020 + (total_vacant_2020 \* (1-unavail_rate)) Vacrate_avail_2020 = 1 - ( hu_occ 2020/ Avail_units_2020)

Calculate Housing Shortage/Surplus for 2020: Hous_short_2020 = (RVR2020 - \[vacrate_avail_2020\]) \* Avail_units_2020 This may be positive (shortage) or negative (surplus)

Calculate vacated units – number of units freed up due to HH decline VacatedHU_20_30 = Max(0, (-1 \* HHchg_20_30))

Calculate vacant units associated with HH growth HHgrowth_vac_20_30 =MAX(HHchg_20_30\* RVR2030, 0)

Discount production needed to close existing shortage Shortage_units_20_30 = IF (Hous_short_2020 \> HHchg_20_30, MAX(HHchg_20_30, (Hous_short_2020 \*0.5)), Hous_short_2020)

Calculate total vacant units needed TotVac_20_30 = Shortage_units_20_30 + HHgrowth_vac_20_30

Calculate total Housing Unit Demand Tot_HU_20_30 = max(max(HHchg_20_30, 0) + TotVac_20_30, 0)

Calculate remaining shortfall for subsequent decade Hous_short_2030 = Hous_short_2020 – Shortage_units_20_30
...
ten <- c('H10_002N','H10_010N') # Total owner-occupied units, Total renter-occupied units
...
d1 <-
  ...
  select(-NAME) %>%
  setDT()
setnames(d1, c('GEOID', 'dhc_units_2020', 'dhc_households_2020', ns, 'own_occ_units', 'rent_occ_units'))
...
dhc <- d1 %>%
  ...
  mutate(
    total_available_units = dhc_households_2020 + Total_For_rent + Total_Rented_not_occupied + Total_For_sale_only + Total_Sold_not_occupied,
    total_available_rental_units = Total_For_rent + Total_Rented_not_occupied + rent_occ_units,
    total_available_owner_units = Total_For_sale_only + Total_Sold_not_occupied + own_occ_units,
    total_available_vacant_units = Total_For_sale_only + Total_For_rent,
    vac_rate = round(total_available_vacant_units / total_available_units, 3),
    rent_vac_rate = round(Total_For_rent / total_available_rental_units,3),
    own_vac_rate = round(Total_For_sale_only / total_available_owner_units,3)
    ) %>% 
  rename(total_vacant_units=Total,
         vacant_available_to_rent=Total_For_rent,
         vacant_rented_not_occ=Total_Rented_not_occupied,
         vacant_available_to_own=Total_For_sale_only,
         vacant_sold_not_occ=Total_Sold_not_occupied,
         vacant_seasonal=Total_For_seasonal_recreational_or_occasional_use,
         vacant_migrant=Total_For_migrant_workers,
         vacant_other=Total_Other_vacant)
