...
#First calculate the number of people with a disability 
people_with_disability <- pums %>%
  group_by(uniq_obs) %>%
  filter(DIS == 1) 

print(people_with_disability)
...
households_with_disability <- people_with_disability %>%
  group_by(SERIALNO) %>%
  summarize(WGTP = first(WGTP)) %>%  #Retain household weights (same for all persons in the household)
  ungroup()

#Define the survey design with household weights
household_design <- svydesign(ids = ~1,           #No clustering
                              weights = ~WGTP,    #Apply household weights
                              data = households_with_disability)

#Calculate the total number of households (weighted)
total_households_with_disability <- svytotal(~I(SERIALNO != 0), design = household_design)
