...
Load CPI deflator time series to adjust PUMS income to 2013 dollars (for UrbanSim)
...
```{r}
# PUMA crosswalk
mxw <- fread(paste0(workpath, 'PUMS_data/ma_muni_puma10_join.csv'))
...
infile <- paste0(workpath, 'Templates/reweighting_config_2021.json')

for (m in mpos){
  outfile <- paste0(workpath, scen, '/Input_Files/', m, '/reweighting_config_2021_', m, '.json')
  cfig = readLines(infile)
  cfig = gsub('PUMS2021', paste0('PUMS2021_', scen), cfig)
  writeLines(cfig, outfile)
}
```
...
source('~/scripts/reweighter/algo_hh.R')

# Configuration Loop
for (mid in mpos){
  conpath <- paste0(workpath, scen, '/Input_Files/', mid)
  source('~/scripts/reweighter/read_config_RPA.R')
...
# Running with MAPC 101 or with MAPC 97 towns
reb <- 'MAPC101'
#reb <- 'MAPC97'
...
for (yr in c(2020,2030,2035,2040,2045,2050)){
  ...
  for (mid in mpos) {
    ...
    target <- fread(paste0(ctrltotal.path,'hhtype.',reb,'.targetfile.',yr,'.csv')) %>% 
                      filter(RPA==mid) %>% 
                      fwrite(paste0(inpath, '/AgeHHder_HHtype.csv'))
    message(paste0(mid, ' targets set'))
    
    pums <- fread(paste0(inpath, 'PUMS',vintage,'_', mid, '.csv'))
    source('scripts/reweighter/Manage_Scenario_RPA.R')
    message(paste0(mid, ' reweighter complete'))
    
    mess <- data.table(txt = paste0(mid, ' reweighter complete'))
    fwrite(mess,
           paste0(outpath, mid, '_', yr, '_complete.txt'))
  }
}
