...
test2 <- subset.2.adj.test |> 
  filter(SPORDER.adj == "1") |> 
  group_by(HHtype.adj) |> 
...
test <- partial.pums_data.subset1 |> select(SERIALNO, SPORDER, SERIALNO.adj, SPORDER.adj) |> 
  mutate(
    SERIALNO.adj.test = str_sub(SERIALNO.adj, start = 1, end = 13),
    
  )

length(unique(test$SERIALNO))
length(unique(test$SERIALNO.adj))
...
for (i in years) {
  ...
  if(i == 2020 | i == 2025){
    ...
    #  adj.test.distinct <- adj |> filter(SPORDER.adj == 1) |> distinct(SERIALNO, .keep_all = TRUE)
    # # adj.test.sn <- adj |> filter(SPORDER.adj == 1) |> select(SERIALNO, SPORDER)
      sample.test <- pums.sample |> filter(sample == i) |> filter(SPORDER.adj == 1)
    # sample.test.sn <- pums.sample |> filter(sample == i) |> filter(SPORDER.adj == 1) |> select(SERIALNO, SPORDER)
    # 
    # x <- full_join(
    #   adj.test.sn,
    #   sample.test.sn,
    ...
    # 
    # 
    # print(base::union(base::setdiff(adj.test.sn, sample.test.sn), base::setdiff(sample.test.sn, adj.test.sn)))
    # 
    # print(base::setdiff(base::union(adj.test.sn, sample.test.sn), base::intersect(adj.test.sn, sample.test.sn)))
    # 
    # 
    # # 
    #  sample.test2 <- pums.sample_2030 |> filter(SPORDER.adj == 1)
     print(sum(adj.test$WGTP))
    # print(sum(adj.test.distinct$WGTP))
     print(sum(sample.test$WGTP))
    # print(sum(sample.test2$WGTP))
    ...
    print(sum(adj.test$WGTP))
     ...
     sample.test <- pums.sample |>
     filter(sample == i | sample == (i - 5) | sample == (i - 10) | sample == (i - 15) | sample == (i - 20)) |> filter(SPORDER.adj == 1)
