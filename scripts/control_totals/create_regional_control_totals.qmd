...
if (scen == "Scenario_1"){ # Scenario 1
  ...
  # Load tenure rates for Scenario 2
  ten.r <- fread(paste0(output_path, "tenure_rates.", scen, ".csv"))
...
if (scen == 'Scenario_2') {
  for (i in yrs) {
    if (i != 2020) {
      ...
      hhtype.base <- fread(paste0(output_path, "hhtype_baseline.", hdshp, ".", yr, ".csv"))
      ten.base <- fread(paste0(output_path, "tenure_baseline.", hdshp, ".", yr, ".csv"))
      ...
      target.file2 <- full_join(ten.base,
                                tmp2,
                                by = c("RPA" = "rpa_acr", "AgeCat", "tenure")) |>
        # Baseline files may not have every age group, household type segment that the projections have.
        # Need to fill in places in the baseline where there are zeros for those segments.
