...
household_renter_burden_recent_immig <- pums %>%
 ...
 mutate(Year.Entry = case_when(      # create year of entry groups
      ...
      YOEP >= 2014 & YOEP <= 2024 ~ "2014 to 2024",
      TRUE ~ "0")) %>%
...
householder_tenure_recent_immig <- pums %>%
 ...
 summarise(
    ...
    .groups = "drop") %>%
 filter(householder_nativity == 2) %>%  # filter for foreign-born householders only
 group_by(year_entry, householder_tenure) %>%  # group by year entry and tenure for summarization
 summarise(
...
```{r}
#Edu attainment for recent immigrants
```
...
```{r}
#Count of hhlds headed by recent immigrants by RPA 
