...
test <- full.pums_data.adj |> 
  filter(SERIALNO == "2017000015740")
...
full.pums_data.adj <- full.pums_data.adj |> 
  mutate(
    ...
    # (0) under 16 or not in labor force
    # (1) in labor force or non-civilian worker
    worker.adj = case_when(
      (ESR == "b" | ESR == 6) ~ 0,
      (ESR == 1 | ESR == 2 | ESR == 3 | ESR == 4 | ESR == 5) ~ 1,
      is.na(ESR) ~ 0
    ),
    # Determine if case counts as person
    person.adj = case_when(
      SPORDER.adj != 0 ~ 1,
      SPORDER.adj == 0 ~ 0
    ),
  ...
  mutate(
    ...
    # Sum the total number of workers in the household
    Worker_Total.adj = sum(worker.adj),
    # Sum the total number of children in the household
    Child_Total.adj = sum(child.adj)
  ) |> 
  ungroup() |> 
  mutate(
    ...
    # (3) three or more workers
    WRKHH.adj = case_when(
      Worker_Total.adj == 0 ~ 0,
      Worker_Total.adj == 1 ~ 1,
      Worker_Total.adj == 2 ~ 2,
      Worker_Total.adj >= 3 ~ 3
      ),
    # Determine if person is not hhder vs hhder of 
    # (0) No workers
    # (1) One worker
    ...
    # (3) Three or more workers
    HHder.adj = case_when(
      SPORDER.adj == 1 & WRKHH.adj == 0 ~ 0,
      SPORDER.adj == 1 & WRKHH.adj == 1 ~ 1,
      SPORDER.adj == 1 & WRKHH.adj == 2 ~ 2,
      SPORDER.adj == 1 & WRKHH.adj == 3 ~ 3,
      SPORDER.adj > 1 ~ 99
    ),
    # Determine what type of household the person is heading or not heading 
    # (1) Householder of Household with Children
    ...
    # (4) Non-Householder in a Household with no children 
    # (5) Householder of a Single Person Household
    HHtype.adj = case_when(
      Child_Total.adj >= 1 & NP.adj > 1 & HHder.adj != 99 ~ 1,
      Child_Total.adj >= 1 & NP.adj > 1 & HHder.adj == 99 ~ 2,
      Child_Total.adj == 0 & NP.adj > 1 & HHder.adj != 99 ~ 3,
      Child_Total.adj == 0 & NP.adj > 1 & HHder.adj == 99 ~ 4,
      NP.adj == 1 & HHder.adj != 99 ~ 5
    )
  )
...
test <- full.pums_data.adj |> 
  filter(adj.flag >= 1 & AGEP %in% c(15, 16, 17, 18, 19) & SPORDER.adj == 1) |> 
  select(SERIALNO) |> 
  pull()

test.df <- full.pums_data.adj |> filter(SERIALNO %in% test & SPORDER.adj == 1) |> filter(SERIALNO == "2017000148487")

conor <- full.pums_data.adj |> filter(SERIALNO == "2018HU1181558")
...
# Print the number of original households that have been altered by the household split code for Subset #1.
print(sum(subset.1.og.test$WGTP))
...
# Print the number of original households that have been altered by the household split code for Subset #2.
print(sum(subset.2.og.test$WGTP))
...
# Print the sum of original households altered by the household split code for both subsets #1 and #2.
print(sum(sum(subset.1.og.test$WGTP), sum(subset.2.og.test$WGTP)))
...
partial.pums_data.subset1 <- full.pums_data.adj |> 
  filter(SPORDER.adj == 1 & TYPEHUGQ == 1 & subset.flag.1 == 1 & subset.flag.2 != 1 & adj.flag == 1)

# Subset of Subset 2 split households.
partial.pums_data.subset2 <- full.pums_data.adj |> 
  filter(SPORDER.adj == 1 & TYPEHUGQ == 1 & subset.flag.1 != 1 & subset.flag.2 == 1 & adj.flag == 2)

# QC - Ensure total number of households in the split data sets matches the correct number identified in the script.
print(sum(partial.pums_data.og$WGTP))
...
sample.size <- round(total_split.hhds*0.51, 0)
...
for (i in years){
  ...
  if (df.row.1 >= sample.size) {
    ...
    # And add them up to an object to record the total number of households as QC.
    sample_sum <- sample_sum + as.numeric(sum(tmp$WGTP))
  ...
  } else if (df.row.1 < sample.size & df.row.1 != 0 & (df.row.2 >= sample.size | df.row.2 < sample.size)) {
    ...
    # Recalculate df.row to pass QC print statement.
    df.row.1 <- as.numeric(nrow(partial.pums_data.subset1))
    df.row.2 <- as.numeric(nrow(partial.pums_data.subset2))
...
# QC Check - tell us if the number of households in the samples matches the number of households we started with.
if (sample_sum == 15371){
  print("Finished sampling PUMS subsets.")
} else{
  print("STOP! Data still exist in the subset dataframes. Please review sampling script.")
...
pums.sample <- bind_rows(pums.sample_2030, pums.sample_2035,pums.sample_2040, pums.sample_2045, pums.sample_2050)
...
check_hdsp.r <- function(df){
  # QC Check - determine if the frequencies in each RP, Age Group crosstab add to 1
  hr_check <- df |>
    group_by(rpa_acr, ageCAT6) |>
    summarise(freq_check = sum(freq.split)) |>
    ungroup() |>
    mutate(flag = ifelse(freq_check != 1, 1, 0))
...
for (i in years) {
  ...
  if(i == 2020 | i == 2025){
    ...
    adj <- full.pums_data.adj |> 
      ...
      filter(SERIALNO.adj %in% sn.adj)
    
    # # QC - 
    og.test <- og |> filter(SPORDER.adj == 1)
    adj.test <- adj |> filter(SPORDER.adj == 1)
    sample.test <- pums.sample |> filter(sample == i) |> filter(SPORDER.adj == 1)
    
    print(sum(og.test$WGTP))
    print(sum(adj.test$WGTP))
    print(sum(sample.test$WGTP))
    
    # Remove unsplit original and split households from standard PUMS dataset by SERIALNO
    tmp_pums <- pums |> filter(!SERIALNO %in% sn)
    
    # QC - Check that the numbers of swapped the rows is correct. 
    row.check <- as.numeric((nrow(pums) - nrow(tmp_pums)) - (nrow(og) + nrow(adj)))
    
    if (row.check == 0){
      print("Proceed")
    } else{
      print("STOP! The number of swapped rows in the tmp_pums dataframe is incorrect.")
    }
    ...
    done_pums <- bind_rows(tmp_pums, og, adj) |>
      mutate(
        SERIALNO.adj = coalesce(SERIALNO.adj, SERIALNO),
        SPORDER.adj = coalesce(SPORDER.adj, SPORDER),
        HHtype.adj = coalesce(HHtype.adj, HHtype)
      )
    
    # Calculate headship rates for this projection year.
    hdsp.r <- create_hdsp.r(df = done_pums)
    
    # Calculate tenure rates for this projection year.
    ten.r <- create_tenure.r(df = done_pums)
    
    # QC - check headship rates do not exceed 1
    check_hdsp.r(hdsp.r)
    
    # Assign the headship rate object to a year specific object name.
    assign(paste0("hdsp.r_", i), hdsp.r)
    
    # Assign the tenure rate object to a year specific object name.
    assign(paste0("ten.r_", i), ten.r)
    
    # save year-specific PUMS dataset for generating baselines.
    fwrite(done_pums, paste0(rwt_path,'PUMS_data/PUMS2021_Scenario_2_',i,'.fix1.csv'))
    ...
    og.test <- og |> filter(SPORDER.adj == 1)
    adj.test <- adj |> filter(SPORDER.adj == 1)
    sample.test <- pums.sample |> filter(sample == i | sample == (i - 5)) |> filter(SPORDER.adj == 1)
    ...
    og.test <- og |> filter(SPORDER.adj == 1)
    adj.test <- adj |> filter(SPORDER.adj == 1)
    sample.test <- pums.sample |> filter(sample == i | sample == (i - 5) | sample == (i - 10)) |> filter(SPORDER.adj == 1)
    ...
    og.test <- og |> filter(SPORDER.adj == 1)
    adj.test <- adj |> filter(SPORDER.adj == 1)
    sample.test <- pums.sample |> filter(sample == i | sample == (i - 5) | sample == (i - 10) | sample == (i -15)) |> filter(SPORDER.adj == 1)
   ...
   # QC - Check that the numbers of swapped the rows is correct. 
    ...
    og.test <- og |> filter(SPORDER.adj == 1)
    adj.test <- adj |> filter(SPORDER.adj == 1)
    sample.test <- pums.sample |> filter(sample == i | sample == (i - 5) | sample == (i - 10) | sample == (i-15) | sample == (i - 20)) |> filter(SPORDER.adj == 1)
...
hdsp.r_scenario2 <- purrr::reduce(
  list(hdsp.r_2030, hdsp.r_2035, hdsp.r_2040, hdsp.r_2045, hdsp.r_2050),
  dplyr::full_join,
  by = c('rpa_acr','ageCAT6','var')
) |> 
  dplyr::rename(
    freq.split.30 = freq.split.x,
    freq.split.35 = freq.split.y,
    freq.split.40 = freq.split.x.x,
    freq.split.45 = freq.split.y.y,
    freq.split.50 = freq.split,
  ) |> 
  mutate(
...
write.csv(
  hdsp.r_scenario2,
  paste0(rwt_path,"Headship_rates/hdsp.r.1721_household.split.paced.fix1.csv"),
  row.names = FALSE
)

# Left join all headship rate dataframes to create single file to export.
ten.r_scenario2 <- purrr::reduce(
  list(ten.r_2030, ten.r_2035, ten.r_2040, ten.r_2045, ten.r_2050),
  dplyr::full_join,
  by = c('rpa_acr','ageCAT6','var')
) |> 
  dplyr::rename(
    freq.split.30 = freq.x,
    freq.split.35 = freq.y,
    freq.split.40 = freq.x.x,
    freq.split.45 = freq.y.y,
    freq.split.50 = freq,
  ) |> 
  mutate(
    # Fill in Zeros where NAs exist from new headship rate groups being created in the split household tabulation.
...
write.csv(
  ten.r_scenario2,
  paste0(rwt_path,"Headship_rates/ten.r.1721_household.split.paced.fix1.csv"),
  row.names = FALSE
)
```
