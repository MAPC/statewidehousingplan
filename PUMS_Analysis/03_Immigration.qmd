...
total_weighted_households <- sum(household_data$WGTP, na.rm = TRUE) # sum weights for total household number
...
household_type_foreign_born <- pums %>%
  filter(RT != "H") %>%   # exclude hhlds because nativity is a person-level data point
  select(uniq_obs, SERIALNO, PUMA, WGTP, HHT, NATIVITY) %>% 
  group_by(SERIALNO) %>% 
  summarise(
    household_type = first(HHT, na.rm = TRUE),  # get the first person's householder ethnicity
    ...
    .groups = "drop") %>%
  group_by(householder_nativity, household_type) %>%  # group by nativity and hispanic status
  summarise(
...
```{r}
#Share of age group that is either foreign born or native 
household_age_by_nativity <- pums %>%              
  ...
  filter(householder_nativity != "Unknown") %>%  # Filter out unknown nativity
...
```{r}
#do race shares in reverse way? % of black householders that are foreign born, etc. 

#do I need to do non-hispanic / hispanic for the race categories - probably 
...
household_year_entry_foreign_born <- pums %>%
 filter(RT != "H") %>%   # exclude hhlds because nativity is a person-level data point
 mutate(Year.Entry = case_when(      # create year of entry groups - might need to change around some
      ...
      TRUE ~ "0")) %>%
 select(uniq_obs, SERIALNO, PUMA, WGTP, NATIVITY, Year.Entry) %>% 
 group_by(SERIALNO) %>% 
 summarise(
...
household_size_foreign_born <- pums %>%
 filter(RT != "H") %>%   # exclude hhlds because nativity is a person-level data point
 select(uniq_obs, SERIALNO, PUMA, WGTP, NATIVITY, NP) %>% 
 group_by(SERIALNO) %>% 
 summarise(
  ...
  mutate(household_size_category = case_when(   # create household size categories
    ...
    household_size < 10 ~ as.character(household_size),  # keep sizes less than 10 as strings
    TRUE ~ "Unknown")) %>%
...
household_children_foreign_born <- pums %>%
 filter(RT != "H") %>%   # exclude hhlds because nativity is a person-level data point
 select(uniq_obs, SERIALNO, PUMA, WGTP, NATIVITY, HUPAC) %>% 
 group_by(SERIALNO) %>% 
 summarise(
...
householder_income_foreign_born <- pums %>%
 filter(RT != "H") %>%   # exclude hhlds because nativity is a person-level data point
 select(uniq_obs, SERIALNO, PUMA, WGTP, NATIVITY, HH.Income.Grp) %>% 
 group_by(SERIALNO) %>% 
 summarise(
...
householder_tenure_foreign_born <- pums %>%
 filter(RT != "H") %>%   # exclude hhlds because nativity is a person-level data point
 select(uniq_obs, SERIALNO, PUMA, WGTP, NATIVITY, TEN) %>% 
 group_by(SERIALNO) %>% 
 summarise(
...
household_renter_burden_foreign_born <- pums %>%
 ...
 mutate(Cost.Burden = case_when(      # create cost burden groups 
      ...
      TRUE ~ "0")) %>%
 select(uniq_obs, SERIALNO, PUMA, WGTP, NATIVITY, Cost.Burden) %>% 
 group_by(SERIALNO) %>% 
 summarise(
...
household_english_foreign_born <- pums %>%
 filter(RT != "H") %>%   # exclude hhlds because nativity is a person-level data point
 select(uniq_obs, SERIALNO, PUMA, WGTP, NATIVITY, ENG) %>% 
 group_by(SERIALNO) %>% 
 summarise(
...
household_language_foreign_born <- pums %>%
 filter(RT != "H") %>%   # exclude hhlds because nativity is a person-level data point
 select(uniq_obs, SERIALNO, PUMA, WGTP, NATIVITY, LANX) %>% 
 group_by(SERIALNO) %>% 
 summarise(
...
household_limited_lang_foreign_born <- pums %>%
 filter(RT != "H") %>%   # exclude hhlds because nativity is a person-level data point
 select(uniq_obs, SERIALNO, PUMA, WGTP, NATIVITY, LNGI) %>% 
 group_by(SERIALNO) %>% 
 summarise(
...
household_world_origin_foreign_born <- pums %>%
 filter(RT != "H") %>%   # exclude hhlds because nativity is a person-level data point
 select(uniq_obs, SERIALNO, PUMA, WGTP, NATIVITY, WAOB) %>% 
 group_by(SERIALNO) %>% 
 summarise(
...
```{r}
#Educational attainment of adult hhld members - do not have the variable in current pums data pull
...
household_foreign_born_summary_table_RPA <- pums_RPA %>%
  filter(RT != "H") %>%   # exclude hhlds because nativity is a person-level data point
  group_by(SERIALNO, rpa_acr) %>%  # group by household 
