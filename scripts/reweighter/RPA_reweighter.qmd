...
pums_data <- fread(paste0(workpath, 'PUMS_data/PUMS', vintage, '_formatted.csv')) |> filter(TYPEHUGQ == "1") |> 
  ...
  pums_data <- fread(paste0(workpath, 'PUMS_data/PUMS', vintage, '.Split_formatted.csv')) |> filter(TYPEHUGQ == "1") |> 
  mutate(AgeCat = ageCAT6)
}
...
for (yr in c(2025,2030,2035,2040,2045,2050)){
  for (mid in mpos) {
    ...
    comp.dt <- rbind(comp.dt,comp.out)
      ...
      pums <- fread(paste0(inpath, 'PUMS', vintage, '_', mid, '.csv'))
    setkey(pums, SERIALNO, SPORDER.adj)
    
    rw <- data.table()
    
    x <- fread(paste0(outpath, 'weights_final_', scen, '_', mid, '_', yr, '.csv')) |> select(SERIALNO, SPORDER.adj, new_WGTP) |> mutate(mpo = mid) |> setDT()
    setkey(x, SERIALNO, SPORDER.adj)
    xp <- pums[x]
    fwrite(xp, paste0(outpath, '/reweighted_PUMS_', scen, '_', reb, '_', mid, '_', hdshp, '_', yr, '.csv'))
    xsum <- xp[SPORDER.adj == 1, lapply(.SD, sum), .(mpo), .SDcols = c('WGTP', 'new_WGTP')]
    xsum[, rw_HH := round(new_WGTP)][, pums_baseline_hh := round(WGTP)]
    rw <- rbind(rw, xsum)
    setkey(rw, mpo)
    comp <- tsum[rw]
