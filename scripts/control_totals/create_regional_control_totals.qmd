...
source("C:/Project_Work/statewidehousingplan/scripts/control_totals/Projections.Functions.R")
...
headship_rates <- pums_data |> 
  ...
  group_by(rpa_acr, ageCAT6, tenure) |> 
...
proj.hh.MAPC101_by.rpa.hhtype <- left_join(
  ...
  relocate(rpa_acr, AgeCat, tenure, HHtype, hh, year)
...
proj.hh.MAPC97_by.mpo.hhtype <- left_join(
  ...
  select(-c(pop, hhpop, freq, var, ageCAT6)) |> 
  relocate(mpo, AgeCat, tenure, HHtype, hh, year)
...
for (i in yrs) {
  ...
  target.file <- left_join(
    hhtype_baseline,
    tmp,
    by = c("RPA" = "rpa_acr", "AgeCat", "tenure", "HHtype")
  ) |> 
    dplyr::rename(
      TARGET = hh
    )
...
for (i in yrs) {
  ...
  target.file <- left_join(
    hhtype_baseline,
    tmp,
    by = c("RPA" = "mpo", "AgeCat", "tenure", "HHtype")
  ) |> 
    dplyr::rename(
      TARGET = hh
    )
