...
for (i in years){
  ...
  if (df.row.1 >= sample.size) {
    ...
    test <- slice_sample(partial.pums_data.subset1.sample, n = sample.size, replace = F)
    print(sum(test$WGTP))
    test_sporder.adj <- test |> pull(SPORDER.adj)
    partial.pums_data.subset1 <- filter(!SPORDER.adj %in% test_sporder.adj)
    
    df.row.1 <- df.row.1 - nrow(test)
    print("Subset 1 has ", df.row.1, " rows remaining.")
    print("Subset 2 has ", df.row.2, " rows remaining.")
    
    assign(test, paste0("pums.sample_",i))
