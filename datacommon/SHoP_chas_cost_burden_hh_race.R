...
# Cost Burdened Households by Race (Municipal)
# Table: hous_hh_race_by_cb_chas_m
...
get_chas_hh <- read.csv(paste0("https://datacommon.mapc.org/csv?table=tabular.hous_hh_race_by_cb_chas_m&database=ds&years=",input_year,"&year_col=acs_year"))
...
map_title = paste0(input_year," White Severely Cost Burdened HH")

whi_scbp_plot <- ggplot(shp_join) +
  geom_sf(aes(fill = whi_scbp), linewidth = 0, alpha = 0.9) +
  theme_void() +
  ...
  labs(
    title = map_title,
    subtitle = "percentage of White HH",
    caption = "Data: CHAS Cost Burdened Households by Race"
  ) +
  theme(
...
map_title = paste0(input_year," African American Severely Cost Burdened HH")

aa_scbp_plot <- ggplot(shp_join) +
  geom_sf(aes(fill = aa_scbp), linewidth = 0, alpha = 0.9) +
  theme_void() +
  scale_fill_viridis_c(
    trans = "log", breaks = c(1, 5, 10, 20, 50, 100),
    name = "pct",
  ...
  labs(
    title = map_title,
    subtitle = "count of African American HH",
    caption = "Data: CHAS Cost Burdened Households by Race"
  ) +
  theme(
    text = element_text(color = "#22211d"),
