...
household_size_dis <- people_with_disability %>%
 ...
 summarise(
  ...
  mutate(household_size_category = case_when(              # create household size categories
    ...
    TRUE ~ "Unknown")) %>%
  group_by(household_size_category) %>%  # group by  size
  summarise(
...
hhld_size_dis_state <- pums %>%
  ...
  group_by(SERIALNO) %>%  
  summarise(
    household_size = first(NP, na.rm = TRUE),  # first person's hhld size
  ...
  mutate(
    percentage_of_total = (total_weight / sum(total_weight, na.rm = TRUE)) * 100,  # calculate percentage
