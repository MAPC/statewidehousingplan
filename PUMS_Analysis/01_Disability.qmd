...
#Calculate the total number in sample (weighted)
person_data <- pums %>%
filter(RT == "P") %>%
filter(TYPEHUGQ != 2)
...
mutate(age_range = case_when(
    ...
    AGEP >= 18 & AGEP < 65 ~ "18-64", #might want to break down age detail more
    AGEP >= 65 ~ "65 and over"))
...
disability_age_counts <- age_people_with_disability %>%
  ...
  mutate(age_range = factor(age_range, levels = c("Under 18", "18-64", "65 and over"))) %>%
  arrange(age_range)

```

The majority of disabilities are in adults. About half, or 48%, of people with any disability are in the age group 18-64. Another 43% is 65+ and only around 8% are under 18. 
