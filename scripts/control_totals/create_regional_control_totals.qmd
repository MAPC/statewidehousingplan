...
tenure_rates <- pums_data |> 
  # Inclues only observations for people in households (omits GQ)                
  filter(TYPEHUGQ == "1" & SPORDER == 1 & !ageCAT6 %in% c("1","2")) |>                     
...
```{r}
# Determine which PUMS data to use as the baseline for the targetting files
if (scen == "Scenario_1"){
...
#===============================================================================
# 7.1.5-7 Baseline targetting files for 2020
...
# 7.5 Load in Headship Rate Scenarios
...
# 7.6 Join Headship Rates to Projected Population in Households by Age Group and RPA
...
# 7.4.2.1 - Household Projections by RPA (MAPC101), 5-Year Age Groups, and Household Type
proj.hh.MAPC101_by.rpa.hhtype <- hdsp.r |>
  left_join(
    umdi.pop.MAPC101, 
    by = c("rpa_acr", "ageCAT6"), 
  ...
  # Filter the headships rates for the three household type categories
  ...
  filter(var == "hhderchild" | var == "hhdernochild" | var == "single") |>
  mutate(
    HHtype = case_when(
      var == "hhderchild" ~ "x == 1",
...
# Sum up households to the RPA, 5-Year Age Category level by Year
...
# 7.4.2.2 - Household Projections by RPA (MAPC101), 5-Year Age Groups, and Tenure
proj.hh.MAPC101_by.rpa.tenure <- tenure_rates |>
  mutate(AgeCat = paste0("x == ", ageCAT6),
         tenure = paste0("x == ", tenure)
         ) |> 
  left_join(
    hhds.MAPC101, 
  ...
  rowwise() |>
  # Calculating the number of households in each RPA and age category
  mutate(hh = round(hh * freq, 0)) |>
  select(-c(freq, ageCAT6)) |>
  relocate(rpa_acr, AgeCat, tenure, hh, year)

#===============================================================================
# 7.4.2.3 Household Projections by MPO (MAPC97) and Household Type
proj.hh.MAPC97_by.mpo.hhtype <- hdsp.r |>
  left_join(
    umdi.pop.MAPC97, 
    by = c("rpa_acr"="mpo", "ageCAT6"), 
    relationship = "many-to-many"
  ) |> 
  rename(mpo=rpa_acr) |> 
  rowwise() |> 
...
# 7.4.2.2 - Household Projections by MPO (MAPC97), 5-Year Age Groups, and Tenure 
proj.hh.MAPC97_by.mpo.tenure <- tenure_rates |>
  mutate(
    AgeCat = paste0("x == ",ageCAT6),
    tenure = paste0("x == ", tenure)
  ) |> 
...
# 7.7 Create Reweighter Template Files for Household Projections by RPA/MPO and Household Type
