...
for (yr in c(2025,2035)){
    d1 <- fread(paste0(workpath,scen,'/Output_Files/reweighted_PUMS_', scen, '_', reb, '_', hdshp, '_', yr, '.csv'))
    pums <- fread(paste0(workpath,'PUMS_data/PUMS2021_Scenario_2_2030.csv')) |> select(SERIALNO,SPORDER,PINCP)
    if(!'PINCP' %in% names(d1)){
      dx <- d1 |> left_join(pums, by=c('SERIALNO','SPORDER'))
    }
    dx[PINCP<0,PINCP:=0]
