setwd('S:/Network Shares/DS Projects/Current_Projects/Projections/Reweighter/Input_Files/')
library(data.table)
library(tidyverse)
library(tidycensus)
...
# Load control totals and sum total households
ux <- fread('subregions_crosswalk.csv') %>% rename(subregion_code = subregion_code_urbansim, mpo = RPA)
ct <- fread('../Output_Files/Statewide_Control_HHds_Projections2050_MB_v03.23.23.csv') %>% filter(year==2020) %>% left_join(ux, by='subregion_code') %>% setDT()
ct <- ct[,lapply(.SD,sum),.(mpo),.SDcols='total_number_of_households']
setkey(ct,mpo)
