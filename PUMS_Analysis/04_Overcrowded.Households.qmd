...
pums <- pums |> 
  ...
  mutate(
    ...
    HH.Income.Grp = case_when(
      HINCP_adj <= 35000 ~ "1",
      HINCP_adj > 35000 & HINCP_adj <= 75000 ~ "2",
      HINCP_adj > 75000 & HINCP_adj <= 125000 ~ "3",
      HINCP_adj > 125000 & HINCP_adj <= 225000 ~ "4",
      HINCP_adj > 225000 ~ "5",
      .default = "0"
    ),
    ...
    HHLDER.RACE.Grp = case_when(
      ...
      HHLDRRAC1P %in% c("3", "4", "5", "7") ~ "AIAN.NHPI Alone",
      HHLDRRAC1P == "6" ~ "Asian Alone",
      HHLDRRAC1P == "7" ~ "SOR Alone",
      HHLDRRAC1P == "8" ~ "Two or More Races Alone",
      .default = "Unknown"
    )
  ) |>
...
BG.OC.TEN.PPB <- OC.TEN.PPB |> 
  ...
  labs(
    title = "Renter Households are More Likely to be Overcrowded /n than Owner Households",
    x = "Tenure",
...
BG.OC.INC.PPB <- OC.INC.PPB |> 
  filter(overcrowded.ppb == "1") |> 
  ggplot(aes(x = HH.Income.Grp, y = pct)) +
  geom_bar(stat = "identity", color = "black", fill = "steelblue") +
  geom_text(aes(label = scales::percent(pct)), vjust = -0.3, size = 3.5) +
  #scale_x_discrete(limits = rev) +
  scale_y_continuous(label = scales::percent) +
