...
rwt.pums.df.join_shelt <- bind_rows(rwt.pums.df.join, shelter.pums_cleaned.year) |> mutate(
    mpo = coalesce(mpo, rpa_acr)
    ) |> 
  setDT()
rwt.pums.df.join_shelt[is.na(children_in_HH),children_in_HH:=Child_Total]

rm(rwt.pums.df.join, shelter.pums_cleaned.year)
gc()
...
# dt <- copy( rwt.pums.df.join_shelt)
# 
...
full.crosstab <- rwt.pums.df.join_shelt  |> 
  # filter(TYPEHUGQ == 1 & SPORDER == 1) |>
  filter(SPORDER == 1) |>
  group_by(
    mpo,
...
```{r}
children_inhhd <- rwt.pums.df.join_shelt |> 
