...
config_file <- paste0("reweighting_config_2020baseyear_",mid,".json")

#
print("Version 11/11/2022")
#
#  Moves the construction of the children list to
...
print("Beginning creation of HH member list...")
t_beg <- Sys.time()
children <- list()

d <- data$SPORDER

for (r in seq(length(d))){
  if (d[r] == 1){
    c_list <- c()
...
t_end <- Sys.time()
print(paste("HH member list complete | Time: ",(t_end-t_beg)))
