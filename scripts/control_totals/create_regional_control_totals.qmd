...
xw <- read.csv(paste0(output_path, 'PUMA10_RPA_crosswalk.csv')) |> 
  mutate(
...
pums_data <- fread(paste0(rwt_path, '/PUMS_data/PUMS', vintage, '_formatted.csv'))
#===============================================================================
...
#===============================================================================
# 7.1.4-6 Baseline targetting files for 2020

# 7.1.4 For 2020 - baseline file of households by age of householder by tenure
...
# However, we can't prdouce exactly the same headship rates by age of householder and household type from Census as we can from PUMS.
...
# 7.4.1 Load in Headship Rate Scenarios
...
# 7.4.2 Join Headship Rates to Projected Population in Households by Age Group and RPA

```{r}
# 7.4.2.1 - Household Projections by RPA (MAPC101) and Household Type
proj.hh.MAPC101_by.rpa.hhtype <- hdsp.r %>%
  mutate(ageCAT6 = as.character(ageCAT6)) %>%
  left_join(umdi.pop.MAPC101, 
            by = c("rpa_acr", "ageCAT6"), 
...
# Add ageCAT4 to UMDI pop in HH data
umdi.pop.ageCAT4.MAPC101 <- umdi.pop.MAPC101 %>% 
  mutate(ageCAT4 = as.numeric(ageCAT6) - 3) %>% 
  select(-ageCAT6) %>% 
  filter(ageCAT4>0)
  
proj.hh.MAPC101_by.rpa.tenure <- tenure_rates %>%
  left_join(umdi.pop.ageCAT4.MAPC101, 
            by = c("rpa_acr", "ageCAT4"), 
            relationship = "many-to-many") |>
  rowwise() |>
  # Calculating the number of households in each RPA and age category
  mutate(hh = round(hhpop * freq, 0),
  ...
  select(-c(pop, hhpop, freq)) |>
  relocate(rpa_acr, ageCAT4, tenure, hh, year)

#===============================================================================
# 7.4.2.2 Household Projections by MPO (MAPC97) and Household Type
proj.hh.MAPC97_by.mpo.hhtype <- hdsp.r %>%
  mutate(ageCAT6 = as.character(ageCAT6)) %>%
  left_join(umdi.pop.MAPC97, 
...
# Add ageCAT4 to UMDI pop in HH data
umdi.pop.ageCAT4.MAPC97 <- umdi.pop.MAPC97 %>% 
  mutate(ageCAT4 = as.numeric(ageCAT6) - 3) %>% 
  select(-ageCAT6) %>% 
  filter(ageCAT4>0)

proj.hh.MAPC97_by.rpa.tenure <- tenure_rates %>%
  left_join(umdi.pop.ageCAT4.MAPC97, 
            by = c("rpa_acr"="mpo", "ageCAT4"), 
            relationship = "many-to-many") |>
  rowwise() |>
  # Calculating the number of households in each RPA and age category
  mutate(hh = round(hhpop * freq, 0),
...
for (i in yrs) {
  ...
  rm(tmp, tmp2, target.file1, target.file2)
...
for (i in yrs) {
  ...
  tmp2 <- proj.hh.MAPC97_by.rpa.tenure |> 
    filter(
      year == i
    ) |> 
    select(-c(year))   
