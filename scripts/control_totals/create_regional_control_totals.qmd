...
if (scen == "Scenario_1"){ # Scenario 1
  ...
  hdsp.r <- hdsp.r_read(scenario = scen, file = "headship.rates.comparison_2000.hsr_to_1721.household.split.hsr.csv") |> 
    select(rpa_acr, ageCAT6, var, freq.scenario2) |> 
    dplyr::rename(freq = freq.scenario2)
  
  hdsp.r <- bind_rows(headship_rates, hdsp.r)
...
proj.hh.MAPC101_by.rpa.hhtype <- NULL
...
for (i in years) {
  ...
  print(paste0("Projecting Households for ",i,"."))
  
  tmp_hdsp.r <- hdsp.r |> filter(year == i)
  
  tmp_proj.hh <- left_join(
    hdsp.r,
    umdi.pop.MAPC101,
    by = c("rpa_acr", "ageCAT6"),
  ...
  proj.hh.MAPC101_by.rpa.hhtype <- bind_rows(proj.hh.MAPC101_by.rpa.hhtype, tmp_proj.hh)
  
  hhds.MAPC101 <- bind_rows(hhds.MAPC101, tmp_hh)
  
  rm(tmp_hdsp.r, tmp_proj.hh, tmp_hh)
