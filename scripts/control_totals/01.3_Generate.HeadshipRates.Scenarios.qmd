...
partial.pums_data.og <- full.pums_data.adj |> 
  filter(SPORDER.adj == 1 & TYPEHUGQ == 1 & subset.flag.1 == 0 & adj.flag == 0)
...
total_split.hhds <- sum(nrow(partial.pums_data.subset1), nrow(partial.pums_data.subset2))
...
for (i in years){
  ...
  df.row.1 <- nrow(partial.pums_data.subset1.sample)
  df.row.2 <- nrow(partial.pums_data.subset2.sample)
  
  if (df.row.1 >= sample.size) {
    
    # Sample from the subset pums records.
    tmp <- slice_sample(partial.pums_data.subset1.sample, n = sample.size, replace = F)
    # Print the number of households that have been sampled.
    print(sum(tmp$WGTP))
    # Create a list of SERIALNO.adj from the sample.
    tmp_sporder.adj <- tmp |> pull(SERIALNO.adj)
    # Remove sampled households from the subset.
    partial.pums_data.subset1 <- filter(!SERIALNO.adj %in% test_sporder.adj)
    
    # Counts the number of rows remaining in each subset.
    rowcount.1 <- df.row.1 - nrow(tmp)
    rowcount.2 <- df.row.2 - nrow(tmp)
    
    # Prints the number of remaining rows in each subset.
    print("Subset 1 has ", rowcount.1, " rows remaining.")
    print("Subset 2 has ", rowcount.2, " rows remaining.")
    
    # Creates a dataframe of the sampled data for loop year.
    assign(tmp, paste0("pums.sample_",i))
    
    # Remove dataframes that need to be recycled in the next loop.
    rm(tmp, tmp_sporder.adj, rowcount.1, rowcount.2)
    ...
    tmp.sample.size.1 <- sample.size - df.row.1
    tmp.sample.size.2 <- sample.size - tmp.sample.size.1
    
    # Sample from the subset pums records. 
    tmp.1 <- slice_sample(partial.pums_data.subset1.sample, n = tmp.sample.size.1, replace = F)
    tmp.2 <- slice_sample(partial.pums_data.subset2.sample, n = tmp.sample.size.2, replace = F)
    ...
    # Create a list of SERIALNO.adj from the sample.
    tmp.1_sporder.adj <- tmp.1 |> pull(SERIALNO.adj)
    tmp.2_sporder.adj <- tmp.2 |> pull(SERIALNO.adj)
    
    # Remove sampled households from the subset.
    partial.pums_data.subset1 <- filter(!SERIALNO.adj %in% tmp.1_sporder.adj)
    partial.pums_data.subset2 <- filter(!SERIALNO.adj %in% tmp.2_sporder.adj)
    
    # Counts the number of rows remaining in each subset.
    rowcount.1 <- df.row.1 - nrow(tmp.1)
    rowcount.2 <- df.row.2 - nrow(tmp.2)
    ...
    # Remove dataframes that need to be recycled in the next loop.
    rm(tmp, tmp.1, tmp.2, tmp.1_sporder.adj, tmp.2_sporder.adj, rowcount.1, rowcount.2)
    
  } else if (df.row.1 == 0 & df.row.2 >= sample.size){
    
    # Sample from the subset pums records.
    tmp <- slice_sample(partial.pums_data.subset2.sample, n = sample.size, replace = F)
    # Print the number of households that have been sampled.
    print(sum(tmp$WGTP))
    # Create a list of SERIALNO.adj from the sample.
    tmp_sporder.adj <- tmp |> pull(SERIALNO.adj)
    # Remove sampled households from the subset.
    partial.pums_data.subset2 <- filter(!SERIALNO.adj %in% test_sporder.adj)
...
# 5.0 Create Headship Rate Generation function
