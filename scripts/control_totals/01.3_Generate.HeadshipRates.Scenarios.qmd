...
for (i in years){
  ...
  if (df.row.1 >= sample.size) {
    ...
    rowcount.1 <- as.numeric(df.row.1 - nrow(tmp))
    rowcount.2 <- as.numeric(df.row.2 - nrow(tmp))
    
    # Prints the number of remaining rows in each subset.
    print(paste0("Subset 1 has ", rowcount.1, " rows remaining."))
    print(paste0("Subset 2 has ", rowcount.2, " rows remaining."))
    
    # Creates a dataframe of the sampled data for loop year.
    assign(paste0("pums.sample_",i), tmp)
    
    # Print Statement so I know which year used which logic.
    print(paste0("Sampling finished for year ", i))
    
    # Remove dataframes that need to be recycled in the next loop.
    rm(tmp, rowcount.1, rowcount.2)
    ...
    # Counts the number of rows remaining in each subset.
    rowcount.1 <- as.numeric(df.row.1 - nrow(tmp.1))
    rowcount.2 <- as.numeric(df.row.2 - nrow(tmp.2))
    ...
    # Remove dataframes that need to be recycled in the next loop.
    rm(tmp, tmp.1, tmp.2, rowcount.1, rowcount.2)
    ...
    # Counts the number of rows remaining in each subset.
    rowcount.1 <- as.numeric(df.row.1)
    rowcount.2 <- as.numeric(df.row.2 - nrow(tmp))
