...
# Create a list of RPA's to cycle through when filtering for the reweighter extracts
rpa.list <- c("BRPC","CCC","FRCOG","CMRPC","MAPC","MVC","MVPC","NMCOG","NPEDC","OCPC","PVPC","SRPEDD")

# For loop to filter PUMS data by RPA for reweighter base data
for (i in rpa.list){
  # Filter PUMS data by RPA
  tmp <- pums_data.adj.rwt |> 
    filter(
      rpa_acr == i
    )
  ...
  write.csv(
    tmp,
    file = paste0(rwt_path,"/Scenario_2/Input_Files/",i,"/Split.PUMS2021_",i,".csv"),
    row.names = FALSE
  )
  
  # Remove tmp file created for a specific RPA
  rm(tmp)
  
  # Print statement 
  print(paste0("Scenario 2 PUMS Extract for ", i, " written."))
}
...
write.csv(
  ...
  paste0(root,"Scenario_2/headship.rates_household.split.csv"),
  row.names = FALSE
)
...
write.csv(
  tenure_rates.split,
  paste0(root,"Scenario_2/tenure.rates_household.split.csv"),
  row.names = FALSE
)
...
# 5.1 Compare Split Household Headship Rates to Current PUMS Vintage Headships Rates
...
# Join 2017-2021 5-year ACS PUMS data Headship Rates to the Split Households Headship Rates
ten.r_1721.compare.1721.split <- full_join(
  tenure_rates,
  tenure_rates.split,
  by = c("rpa_acr","ageCAT6","var")
) |> 
...
# Write the 1721 to 1721 Split Tenure Rate Comparison file.
write.csv(
  ten.r_1721.compare.1721.split,
  paste0(root,"Scenario_2/tenure.rates.comparison_1721.tr_to_1721.household.split.tr.csv"),
  row.names = FALSE
)
```

# 6.0 Compare 2000 Headship Rates and 1721 Split Headship and Tenure Rates
...
# 6.2 Tenure Rate Comparison by RPA, Age Category, and Tenure Group
```{r}
#===============================================================================
# 6.2 Tenure Rate Comparison by RPA, Age Category, and Tenure Group
# Load in 2000 Tenure rates generated by this script (C:/Project_Work/statewidehousingplan/scripts/control_totals/01.1_Generate.2000.HeadshipRates.qmd)
ten.r_2000 <- read_csv(paste0(rwt_path,"Headship_rates/hdsp.r_2000_RPA.AgeCat.Tenure.csv")) |> 
  mutate(
    ageCAT6 = as.factor(ageCAT6)
  )

# Compare Headship Rates from 2000 5% PUMS and Headship Rates from 17-21 ACS 5-year with Split Households by RPA, Age Category, and Household Type
ten.r_2000.compare.1721.split <- full_join(
  ten.r_2000,
  tenure_rates.split,
  by = c("rpa_acr","ageCAT6","var")
) |> 
...
# QC Check - Determine if the Scenario 2 headship rate mix remains less than 1.
ten_check <- ten.r_2000.compare.1721.split |> 
  group_by(
    rpa_acr,
    ageCAT6
  ) |> 
...
# Write to .csv
write.csv(
  ten.r_2000.compare.1721.split,
  paste0(root,"Scenario_2/ten.rates.comparison_2000.tr_to_1721.household.split.tr.csv"),
  row.names = FALSE
)
```
