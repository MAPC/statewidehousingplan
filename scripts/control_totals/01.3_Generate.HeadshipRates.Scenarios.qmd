...
for (i in years){
  ...
  df.row.1 <- nrow(partial.pums_data.subset1)
  df.row.2 <- nrow(partial.pums_data.subset2)
  ...
  if (df.row.1 >= sample.size) {
    ...
    tmp_serialno.adj <- tmp |> pull(SERIALNO.adj)
    # Remove sampled households from the subset.
    partial.pums_data.subset1 <- filter(!SERIALNO.adj %in% tmp_serialno.adj)
    ...
    rm(tmp, tmp_serialno.adj, rowcount.1, rowcount.2)
    ...
    # Create a list of SERIALNO.adj from the sample.
    tmp.1_serialno.adj <- tmp.1 |> pull(SERIALNO.adj)
    tmp.2_serialno.adj <- tmp.2 |> pull(SERIALNO.adj)
    
    # Remove sampled households from the subset.
    partial.pums_data.subset1 <- filter(!SERIALNO.adj %in% tmp.1_serialno.adj)
    partial.pums_data.subset2 <- filter(!SERIALNO.adj %in% tmp.2_serialno.adj)
    ...
    # Remove dataframes that need to be recycled in the next loop.
    rm(tmp, tmp.1, tmp.2, tmp.1_serialno.adj, tmp.2_serialno.adj, rowcount.1, rowcount.2)
    ...
    tmp_serialno.adj <- tmp |> pull(SERIALNO.adj)
    # Remove sampled households from the subset.
    partial.pums_data.subset2 <- filter(!SERIALNO.adj %in% tmp_serialno.adj)
    ...
    # Remove sampled households from the subset.
    partial.pums_data.subset2 <- filter(!SERIALNO.adj %in% tmp_sporder.adj)
