...
# 3.5 Analysis of Subset Households
...
# 6.0 Compare 2000 Headship Rates to

```{r}
# Load in spreadsheet with all previously calculated headship rates.
hdsp.r_2000 <- read.csv(paste0(rwt_path,"Headship_rates/headship_rate_comparison.csv")) |> 
  select(
    ageCAT6,
    head_child2000,
    head_nochild2000,
  ...
  dplyr::rename(
    hhderchild = head_child2000,
    hhdernochild = head_nochild2000,
    single = head_single2000
  ) |> 
  pivot_longer(
    !ageCAT6,
    names_to = "var",
...
# Generating headship rates by RPA and 5-year age group for the ACS 2017-2022 PUMS
headship_rates.1721.split <- pums_data.adj |> 
  # Includes only observations for people in households (omits GQ)                       
  filter(TYPEHUGQ == "1") |> 
  # Groups by variables in the dataframe necessary to getting the granularity of
  # data we're interested in.
  ...
  dplyr::rename(
    freq.1721.split = freq
  )

# QC Check - determine if the frequencies in each RP, Age Group crosstab add to 1
hr_check <- headship_rates.1721.split |> 
  group_by(
    ageCAT6
  ) |> 
  summarise(
    freq_check = sum(freq.1721.split)
  ) |> 
  ungroup() |> 
  mutate(
    flag = ifelse(freq_check != 1, 1, 0)
...
# Set up for comparison to 2000 headship rates.
headship_rates.1721.split <- headship_rates.1721.split  |> 
  filter(
    # Filter for the three household types.
    var %in% c("hhderchild", "hhdernochild", "single")
  )

# Join to 2000 headship rate dataframe
hdsp.r_2000_1721.split_comparison <- full_join(
  hdsp.r_2000,
  headship_rates.1721.split,
  by = c("ageCAT6", "var")
) |> 
  mutate(
    diff = freq.1721.split-freq.2000
  )

# Write to .csv
write.csv(
  hdsp.r_2000_1721.split_comparison,
...
# 6.1 Compare 2000 Headship Rates and 1721 Split Headship Rates by RPA, Age Category, and Household Type
```{r}
hdsp.r_2000 <- read.csv(paste0(rwt_path,"Headship_rates/hdsp.r_2000_RPA.AgeCat.HHtype.csv")) |> 
  filter(
    # Filter for the three household types.
    var %in% c("hhderchild", "hhdernochild", "single")
  ) |> 
...
hdshp.r_2000.compare.1721split <- full_join(
  hdsp.r_2000,
  hdsp.r_1721.split,
  by = c("rpa_acr","ageCAT6","var")
) |> 
  mutate(
    ...
    use.flag = case_when(
      freq.diff < 0 ~ "Use freq.split",
      freq.diff >= 0 ~ "Use freq.2000" 
    )
  )
